...
test('v093 cloud capture default poll interval is reduced to avoid Supabase/API pressure', () => {
  const timers = []
...
test('v093 cloud capture control endpoints require configured control token', async () => {
  const app = createApp({
    port: 0,
    autoConnect: false,
    deployMode: 'cloud',
...
test('v093 control endpoints reject non-allowed Origin when configured', async () => {
  const app = createApp({
    port: 0,
    autoConnect: false,
    deployMode: 'cloud',
