...
test('Formal.12 raw lane and HTTP deadline fail before the upstream edge while ancillary snapshot writes stay outside the ACK await path', () => {
  ...
  assert.match(writer, /createPool\(resolvedStrategyPoolMax[\s\S]*connectionTimeoutMillis: 5000, queryTimeoutMs: 40000, statementTimeoutMs: 35000/)
  assert.match(writer, /criticalQueryTimeoutMs = 9000[\s\S]*controlQueryTimeoutMs = 8000/)
  assert.match(writer, /maxCriticalConcurrent:\s*sharedBackendPool \? 2 : 1/)
