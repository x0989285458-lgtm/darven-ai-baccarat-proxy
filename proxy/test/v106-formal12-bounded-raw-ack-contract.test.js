...
test('Formal.12 raw lane and HTTP deadline fail before the upstream edge while ancillary snapshot writes stay outside the ACK await path', () => {
  ...
  const deferredTask = server.indexOf('afterResponseTask = () => {', ackStart)
  const ancillarySchedule = server.indexOf('scheduleCaptureAncillaryPersistence({', deferredTask)
  const ackResponse = server.indexOf('const response = jsonResponse(200, ack, frontendOrigin)', ancillarySchedule)
