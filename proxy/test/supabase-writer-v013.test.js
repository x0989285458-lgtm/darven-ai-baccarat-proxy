...
test('v050 low-performing table keeps banker/player prediction and records all-MT equal strategy', () => {
  ...
  assert.equal(prediction.strategy_version, 'v087_最佳權重收斂版')
  ...
  assert.equal(prediction.short_run_adjustment.rule, 'v087_最佳權重收斂版')
})
...
test('v050 high-performing table still keeps confidence in 30-80 range', () => {
  ...
  assert.equal(neutralPrediction.strategy_version, 'v087_最佳權重收斂版')
