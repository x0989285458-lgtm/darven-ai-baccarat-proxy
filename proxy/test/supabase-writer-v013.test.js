...
test('v050 low-performing table keeps banker/player prediction and records all-MT equal strategy', () => {
  ...
  assert.equal(prediction.strategy_version, 'v080_高勝率主權重副預測修正版')
  ...
  assert.equal(prediction.short_run_adjustment.rule, 'v080_高勝率主權重副預測修正版')
})
...
test('v050 high-performing table still keeps confidence in 30-80 range', () => {
  ...
  assert.equal(neutralPrediction.strategy_version, 'v080_高勝率主權重副預測修正版')
