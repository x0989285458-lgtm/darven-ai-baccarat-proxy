...
test('v050 low-performing table keeps banker/player prediction and records all-MT equal strategy', () => {
  ...
  assert.equal(prediction.short_run_adjustment.rule, 'v068_independent_side_weight_profiles')
})
...
test('v050 high-performing table still keeps confidence in 30-80 range', () => {
  ...
  assert.equal(neutralPrediction.strategy_version, 'v068_independent_side_weight_profiles')
