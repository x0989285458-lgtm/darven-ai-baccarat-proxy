...
test('v050 low-performing table keeps banker/player prediction and records all-MT equal strategy', () => {
  ...
  assert.equal(prediction.strategy_version, 'v084_副預測比例收斂版')
  ...
  assert.equal(prediction.short_run_adjustment.rule, 'v084_副預測比例收斂版')
})
...
test('v067 main and side strategy uses high-hit weighted features', () => {
  ...
  assert.equal(ALL_MT_EQUAL_SIDE_WEIGHTS.pair_risk, 0.05)
  assert.equal(ALL_MT_EQUAL_SIDE_WEIGHTS.remaining_rank_pressure, 0.45)
...
test('v050 high-performing table still keeps confidence in 30-80 range', () => {
  ...
  assert.equal(neutralPrediction.strategy_version, 'v084_副預測比例收斂版')
