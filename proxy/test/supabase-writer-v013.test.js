...
test('v050 low-performing table keeps banker/player prediction and records all-MT equal strategy', () => {
  ...
  assert.equal(prediction.strategy_version, 'v079_主預測校正副預測放寬版')
  ...
  assert.equal(prediction.short_run_adjustment.rule, 'v079_主預測校正副預測放寬版')
})
...
test('v050 high-performing table still keeps confidence in 30-80 range', () => {
  ...
  assert.equal(neutralPrediction.strategy_version, 'v079_主預測校正副預測放寬版')
