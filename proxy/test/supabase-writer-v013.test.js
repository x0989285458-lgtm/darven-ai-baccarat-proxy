...
test('v050 low-performing table keeps banker/player prediction and records all-MT equal strategy', () => {
  ...
  assert.equal(prediction.strategy_version, 'v086_主預測實戰偏移校正版')
  ...
  assert.equal(prediction.short_run_adjustment.rule, 'v086_主預測實戰偏移校正版')
})
...
test('v067 main and side strategy uses high-hit weighted features', () => {
  ...
  assert.equal(ALL_MT_EQUAL_SIDE_WEIGHTS.pair_risk, 0.10)
...
test('v050 high-performing table still keeps confidence in 30-80 range', () => {
  ...
  assert.equal(neutralPrediction.strategy_version, 'v086_主預測實戰偏移校正版')
