...
test('v050 low-performing table keeps banker/player prediction and records all-MT equal strategy', () => {
  ...
  assert.equal(prediction.strategy_version, 'v081_五路問路路單走勢主副預測版')
  ...
  assert.equal(prediction.short_run_adjustment.rule, 'v081_五路問路路單走勢主副預測版')
})
...
test('v050 high-performing table still keeps confidence in 30-80 range', () => {
  ...
  assert.equal(neutralPrediction.strategy_version, 'v081_五路問路路單走勢主副預測版')
