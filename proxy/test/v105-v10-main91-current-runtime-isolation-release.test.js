...
const baseCommit = '0929cdbfbe01a1f2a53831cfd1231dc8d4442376'
const releaseTag = 'v105-v10-main.91'
const manifestPath = 'release/v105-v10-main91-current-runtime-isolation-release-manifest.json'
...
test('Main91 is the exact bounded candidate over immutable Main90', () => {
...
test('Main91 binds every present candidate path to immutable Git blob identity', () => {
...
test('Main91 deployment contract is DB-first and keeps the reconciliation additive and fail-closed', () => {
  ...
  const proxyIndex = manifest.deploymentOrder.indexOf('proxy-v1.0.72-cutover')
...
test('Main91 current identities and canonical test gates replace obsolete release truth', () => {
  assert.equal(manifest.releaseVersion, releaseTag)
  assert.deepEqual(manifest.componentVersions, { proxy: '1.0.72', worker: '1.0.66' })
  ...
  const workflow = readFileSync(path.join(root, '.github/workflows/trusted-release-images-main91.yml'), 'utf8')
  assert.match(workflow, /refs\/tags\/v105-v10-main\.91/)
  ...
  assert.match(workflow, /main91-coherent-release-receipt/)
