...
test('release scope freezes one existing session as API-only canonical capture', () => {
  assert.equal(manifest.releaseVersion, 'v105-shadow-v10.11')
    assert.equal(manifest.gitTag, 'v105-shadow-v10.11')
    assert.equal(manifest.applicationVersion, '1.0.48')
...
test('Reviewer P1 attestation exact binding rejects old tag and wrong tree while accepting the dynamic candidate index tree', () => {
  ...
  const attestation = {
    ...
    shadowV10MigrationSha256: 'e'.repeat(64),
