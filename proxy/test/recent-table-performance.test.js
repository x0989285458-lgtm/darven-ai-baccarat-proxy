...
test('v098.11 hydrates and updates an 18-round settled real-card performance window per table', () => {
  ...
  const rows = Array.from({ length: 22 }, (_, index) => ({
    ...
    strategy_version: index < 10 ? 'v097_副預測命中校準與門檻降5版' : 'v098_主信心實際命中校準版',
  ...
  store.record({ table_id: 'BAG01', shoe_no: '1', round_no: 23, strategy_version: 'v098_主信心實際命中校準版', predicted_result: 'player', actual_result: 'banker', created_at: '2026-07-14T01:00:00.000Z' })
...
test('v098.11 proxy warms settled performance before creating a live prediction', async (t) => {
  ...
  const supabaseClient = {
    ...
    getRuntimeStatus: () => ({ ready: true, degraded: false, activeStrategyVersion: 'v098_主信心實際命中校準版' }),
