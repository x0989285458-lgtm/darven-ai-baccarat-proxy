...
test('infers new rounds from table snapshot deltas and keeps previous table as prediction context', async () => {
  const events = []
...
test('infers new rounds from round number and bead road even when aggregate counts do not change', async () => {
  const events = []
  const state = createProxyState({ onRoundEvent: async (round, table) => events.push({ round, table }) })
...
test('does not infer a burst of fake banker rounds from synthetic zero-count table state', async () => {
  const events = []
  const state = createProxyState({ onRoundEvent: async (round, table) => events.push({ round, table }) })
...
test('live snapshot shoe and round are not overwritten by older lastRound state', async () => {
...
test('prediction result row stores main and side prediction/actual learning payload', () => {
...
test('admin cloud-data status exposes today captured round count', async () => {
  const app = createApp({
    autoConnect: false,
    deployMode: 'cloud',
