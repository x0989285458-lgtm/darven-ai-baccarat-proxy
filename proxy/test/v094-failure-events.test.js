...
test('v094 classifies operational events into capture/write/monitor/control layers', () => {
...
test('v094 worker disconnect is recorded as capture error without leaking token', async () => {
...
test('v094 Supabase write failure is recorded as write error while keeping fresh worker data', async () => {
  const state = createFakeState()
  const client = createCloudCaptureClient({
    ...
    fetchImpl: async () => ({
      ok: true,
      status: 200,
      json: async () => ({ buildVersion: '098', connected: true, authenticated: true, tables: [{ tableId: 'BAG01', round: 7 }] }),
    }),
  })
...
test('v094 stale cloud data is exposed as monitor error on status', async () => {
  const app = createApp({
    autoConnect: false,
    deployMode: 'cloud',
...
test('v094 unauthorized control API is recorded as control error', async () => {
  const app = createApp({
    autoConnect: false,
    deployMode: 'cloud',
    captureSource: 'cloud_browser',
