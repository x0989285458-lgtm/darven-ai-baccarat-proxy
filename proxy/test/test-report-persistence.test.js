...
test('builds cloud memory report row input from 300-round live report', () => {
...
test('persists final report to online memory center when client is configured', async () => {
  const calls = []
  ...
  assert.equal(calls[0].report.strategyVersion, 'v104')
})
