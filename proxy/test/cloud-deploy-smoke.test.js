...
test('cloud deployment smoke checks health, worker status, tick, and worker payload', async () => {
  const calls = []
...
test('cloud deployment smoke reports failures without throwing', async () => {
  const result = await runCloudDeploySmoke({
    apiBaseUrl: 'https://api.example.com',
    expectedVersion: '042',
