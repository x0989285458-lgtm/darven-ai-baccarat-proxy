...
test('V10 writer independently issues, reads, settles, reads compact history, and reads its zeroed counter', async () => {
  ...
  const client = createSupabaseIngestionClient({
    ...
    fetchImpl: async (url) => {
      ...
      if (parsed.pathname.endsWith('/rpc/issue_v105_shadow_v10_prediction')) {
        return response({ prediction_id: 'v10-id', prediction_issued_at: '2026-08-02T01:00:00.000Z', prediction: candidate })
      }
      if (parsed.pathname.endsWith('/v105_shadow_v10_issuances')) {
      ...
      if (parsed.pathname.endsWith('/rpc/settle_v105_shadow_v10_prediction')) return response({ prediction_id: 'v10-id', settlement_sequence: 1 })
      if (parsed.pathname.endsWith('/v105_shadow_v10_sequence_counters')) return response([{ settlement_count: 0 }])
      return response([])
    },
  })
