...
test('v040 server starts cloud capture worker when cloud_browser source has URL', async () => {
  const app = createApp({
    ...
    fetchImpl: async () => ({
      ok: true,
      status: 200,
      json: async () => ({ buildVersion: '100', connected: true, authenticated: true, sessionId: 'server-cloud-1', tables: [{ tableId: 'BAG09' }] }),
    }),
  })
...
test('v052 server honors AUTO_CONNECT=true env for cloud capture on Render', async () => {
  ...
  const app = createApp({
    ...
    fetchImpl: async () => ({
      ok: true,
      status: 200,
      json: async () => ({ buildVersion: '100', connected: true, authenticated: true, sessionId: 'server-cloud-env', tables: [{ tableId: 'BAG10' }] }),
    }),
  })
