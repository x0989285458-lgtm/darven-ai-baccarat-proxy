...
test('v105 active frontend proxy worker protocol and deployment surfaces match', () => {
  ...
  assert.match(read('frontend/package.json'), /"version":\s*"1\.0\.46"/)
  assert.match(read('proxy/package.json'), /"name":\s*"draven-mt-data-proxy-v105"[\s\S]*"version":\s*"1\.0\.46"/)
  ...
  assert.match(read('cloud-browser-worker/package.json'), /"version":\s*"1\.0\.46"/)
