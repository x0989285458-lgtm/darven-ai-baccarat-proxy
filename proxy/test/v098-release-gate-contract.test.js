...
test('v098 pending snapshot contains all pre-result probability score MT derived and card features and settlement only appends outcome fields', () => {
  ...
  assert.equal(pending.buildVersion, '098.23')
...
test('v098 expired pending is deleted by API and the same target is tombstoned instead of rebuilt', async () => {
  let clock = 1_000_000
  let writes = 0
  const app = createApp({
    autoConnect: false,
...
test('v098 cloud worker fetch rejects redirects', async () => {
  let init
  const client = createCloudCaptureClient({
    ...
    fetchImpl: async (_url, options) => {
      init = options
      return { ok: true, json: async () => ({ buildVersion: '098', tables: [], rounds: [] }) }
    },
  })
...
test('v098 preserves the approved v097 Dragon Bonus actual-result rule', () => {
...
test('v098 formal stable scripts aggregate saved rows and never invoke a predictor', () => {
  for (const name of ['stable-capture-report.mjs', 'live-200-report.mjs']) {
