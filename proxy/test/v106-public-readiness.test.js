...
const expected = {
  ...
  expectedRelease: 'v106.0.0-formal.15',
  expectedPackage: '1.0.72',
...
test('Formal.15 readiness blocks an older v106 release even when public health is HTTP 200', async () => {
  const probes = []
  await assert.rejects(verifyV106PublicReadiness({
    ...expected, attempts: 2,
    fetchImpl: async () => response({
      ok: true, version: 'v106', buildVersion: 'v106',
      releaseVersion: 'v106.0.0-formal.15', packageVersion: '1.0.72', commit: 'b'.repeat(40),
    }),
...
test('Formal.15 readiness requires two consecutive exact public identities and resets after any mismatch', async () => {
...
test('Formal.15 readiness rejects an unbounded or incomplete identity contract before network access', async () => {
  let calls = 0
  await assert.rejects(verifyV106PublicReadiness({
