...
test('proxy health and status expose the same build version', async () => {
  ...
  assert.equal(health.releaseVersion, 'v106.0.0-formal.60')
  assert.equal(status.releaseVersion, 'v106.0.0-formal.60')
  assert.equal(health.packageVersion, '1.0.117')
  assert.equal(status.packageVersion, '1.0.117')
