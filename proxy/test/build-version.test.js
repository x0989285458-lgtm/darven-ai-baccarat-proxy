...
test('proxy health and status expose the same build version', async () => {
  ...
  assert.equal(health.releaseVersion, 'v106.0.0-formal.16')
  assert.equal(status.releaseVersion, 'v106.0.0-formal.16')
  assert.equal(health.packageVersion, '1.0.73')
  assert.equal(status.packageVersion, '1.0.73')
