...
test('proxy health and status expose the same build version', async () => {
  ...
  assert.equal(health.releaseVersion, 'v106.0.0-formal.15')
  assert.equal(status.releaseVersion, 'v106.0.0-formal.15')
  assert.equal(health.packageVersion, '1.0.72')
  assert.equal(status.packageVersion, '1.0.72')
