...
test('member frontend login uses memberAccount plus verificationPassword payload', async () => {
  const queries = []
...
test('backend/admin login uses agentAccount only as the public identifier', async () => {
  const queries = []
  const pool = { async query(sql, params = []) { queries.push({ sql, params }); return fakeAuthResult(sql, params) } }
  const client = createLicenseAdminClient({ pool })
...
test('server exposes separated member-login and agent-login endpoints', async () => {
  const calls = []
