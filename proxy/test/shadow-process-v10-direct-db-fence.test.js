...
test('createSupabaseIngestionClient constructs one reusable Direct DB pool with scheduler lane reservations', () => {
  const poolConfigs = []
  ...
  assert.deepEqual(poolConfigs.map((config) => config.max), [10])
...
test('required worker retains Direct DB pool while V10 scrubs every Direct DB env before writer construction', () => {
  ...
  assert.equal(poolConfigs[0].max, 3, 'required V9 pool must stay bounded at three Direct DB connections')
...
test('V10 scoped worker uses only PostgREST and RPC for issuance, read, Final, counter, and history', async () => {
  ...
  const writer = createShadowProcessWriter({
    ...
    fetchImpl: async (url) => {
      ...
      if (path.endsWith('/v105_shadow_v10_rank_sync_issuances')) {
      ...
      if (path.endsWith('/rpc/settle_v105_shadow_v10_rank_sync_prediction')) return response({ prediction_id: 'v10-id', settlement_sequence: 1 })
      if (path.endsWith('/v105_shadow_v10_rank_sync_sequence_counters')) return response([{ settlement_count: 1 }])
      return response([])
    },
  })
