...
test('weak-table strategy applies explicit deweight status and caps confidence below 55', () => {
...
test('reverse correction activates only for weak table with three misses and opposite road signal', () => {
  const tracker = createTablePerformanceTracker()
  ;[
    ['莊', '閒'], ['莊', '閒'], ['莊', '閒'],
  ].forEach(([prediction, winner]) => tracker.record({ prediction, winner }))
...
test('strong table applies conservative boost while keeping confidence within 70 cap', () => {
  const tracker = createTablePerformanceTracker()
  ;[
