...
test('fills only a missing big road when authoritative bead plate already has three contiguous Final rounds', () => {
...
test('builds a temporary road from contiguous exact real rounds when a new-shoe MT table is sparse', () => {
  const state = createProxyState()
  state.setTables([{
...
test('does not publish table updates from completed-round ingestion and cannot create post-result predictions', () => {
  const tableUpdates = []
...
test('preserves a real-round fallback road across newer sparse snapshots in the same shoe', () => {
  const state = createProxyState()
  state.setTables([{
    tableId: 'BAG01', displayName: 'MT百家樂第1桌', tableType: 'BAC',
...
test('lays out long runs on the six-row big road and does not create a standalone tie cell', () => {
  const state = createProxyState()
  state.setTables([{
    tableId: 'BAG01', displayName: 'MT百家樂第1桌', tableType: 'BAC',
    shoe: 14504, round: 1, bankerCount: 0, playerCount: 0, tieCount: 0,
...
test('lets a complete MT road replace the temporary fallback road', () => {
  const state = createProxyState()
  state.setTables([sparseTable({ shoe: 14504, round: 1 })])
  state.upsertRoundEvent(exactRound({ round: 1, winner: 'player' }))
...
test('never carries a fallback road into a new shoe', () => {
  const state = createProxyState()
  state.setTables([sparseTable({ shoe: 14504, round: 1 })])
  state.upsertRoundEvent(exactRound({ round: 1, winner: 'player' }))
...
test('fails closed when exact real rounds are not contiguous from round one', () => {
  const state = createProxyState()
...
test('rejects zero placeholders and non-exact round payloads from road fallback', () => {
  const state = createProxyState()
  state.setTables([sparseTable({ shoe: 14504, round: 1 })])
...
test('rejects provisional show_poker and unknown actions from fallback history', () => {
...
test('fails closed on conflicting exact Final events for the same round', () => {
  const state = createProxyState()
  state.setTables([sparseTable({ shoe: 14504, round: 1 })])
  state.upsertRoundEvent(exactRound({ round: 1, winner: 'player' }))
...
test('rejects delayed previous-shoe events without regressing live table identity or road', () => {
  const state = createProxyState()
  state.setTables([sparseTable({ shoe: 14505, round: 1 })])
...
test('records out-of-order same-shoe history without regressing round or lastRound', () => {
  const state = createProxyState()
...
test('prunes fallback history when a table leaves the active snapshot', () => {
  const state = createProxyState()
  state.setTables([sparseTable({ shoe: 14504, round: 1 })])
  state.upsertRoundEvent(exactRound({ round: 1, winner: 'player' }))
  state.setTables([])
...
test('rejects impossible round numbers from fallback history', () => {
  const state = createProxyState()
  state.setTables([sparseTable({ shoe: 14504, round: 1 })])
...
test('rejects malformed values in all ten raw-result positions', () => {
