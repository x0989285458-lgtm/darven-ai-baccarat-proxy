...
test('writer issuance returns the first durable immutable payload and prediction id', async () => {
  ...
  assert.match(requests[0].url, /\/rpc\/issue_v101_prediction$/)
...
test('writer settles by prediction_id and suppresses an identical duplicate settlement', async () => {
  ...
  assert.match(requests[0].url, /\/rpc\/settle_v101_prediction$/)
...
test('v101 readers require complete current final columns and exclude compatibility and pending rows', async () => {
  const requests = []
  const compatibility = { id: 'compatibility', table_id: 'BAG01', shoe_no: '88', round_no: 3, strategy_version: 'v101', predicted_result: 'banker', actual_result: 'banker', is_hit: true, settlement_final: null, side_hits: null, prediction_features: { prediction_timing: 'pre_result_context', settlement_final: true, side_hits: { tie: false } }, created_at: '2026-07-16T01:00:00Z' }
  ...
  for (const url of requests) {
    ...
    assert.equal(url.searchParams.get('strategy_version'), 'eq.v101')
