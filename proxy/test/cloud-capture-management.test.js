...
test('cloud capture management endpoints expose status, tick, start, and stop', async () => {
  let fetchCount = 0
  const app = createApp({
    port: 0,
    autoConnect: false,
    ...
    fetchImpl: async () => {
      ...
      return {
        ok: true,
        status: 200,
        json: async () => ({ buildVersion: '100', connected: true, authenticated: true, sessionId: 'v041-session', tables: [{ tableId: 'BAG01' }] }),
      }
    },
  })
...
test('cloud capture management refuses start when worker URL is missing', async () => {
