...
test('v098.13 snapshot keeps fallback tables but no duplicate table_summary payload', () => {
...
test('v098.13 snapshot writer updates one latest row through the bounded RPC', async () => {
  const requests = []
...
test('v098.13 snapshot writer limits large fallback payloads to a sixty-second heartbeat', async () => {
  let nowMs = 0
  const requests = []
  const client = createSupabaseIngestionClient({
...
test('v098.13 capture status writes on state change or sixty-second heartbeat only', async () => {
  let nowMs = 0
  const requests = []
  const client = createSupabaseIngestionClient({
    url: 'https://example.supabase.co', serviceKey: 'sb_secret_test_key', now: () => nowMs,
