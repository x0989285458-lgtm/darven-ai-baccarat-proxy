...
test('v104 writer uses only its dedicated issuance RPC and preserves lock diagnostics in immutable payload', async () => {
  ...
  assert.match(requests[0].url, /\/rpc\/issue_v104_iteration_shadow_v5_prediction$/)
  assert.equal(requests[0].body.p_prediction.strategy_version, 'v104-seven-head-shadow-v5-best-stage-side-reweight')
...
test('v104 history reads its dedicated view including unsettled issuance rows for restart hydration', async () => {
  const requests = []
  const row = { strategy_version: 'v104-seven-head-shadow-v5-best-stage-side-reweight', prediction_timing: 'pre_result_context', prediction_issued_at: '2026-07-21T10:00:00Z', settlement_final: false }
  const client = createSupabaseIngestionClient({
    url: 'https://example.supabase.co', serviceKey: 'test-only', requireVerifiedStrategy: false,
  ...
  assert.match(requests[0].pathname, /\/rest\/v1\/v104_iteration_shadow_v5_history$/)
  assert.equal(requests[0].searchParams.get('strategy_version'), 'eq.v104-seven-head-shadow-v5-best-stage-side-reweight')
...
test('v104 iteration history paginates beyond the Supabase 1000-row response cap', async () => {
  const requests = []
  const makeRow = (index) => ({ strategy_version: 'v104-seven-head-shadow-v5-best-stage-side-reweight', prediction_timing: 'pre_result_context', prediction_issued_at: `2026-07-21T10:${String(index % 60).padStart(2, '0')}:00Z`, prediction_id: `p-${index}` })
  const client = createSupabaseIngestionClient({
    url: 'https://example.supabase.co', serviceKey: 'test-only', requireVerifiedStrategy: false,
    fetchImpl: async (url) => {
...
test('v104 durable reports and suggestions paginate beyond the REST 1000-row cap', async () => {
  ...
  const client = createSupabaseIngestionClient({
    ...
    fetchImpl: async (url) => {
      const parsed = new URL(url); requests.push(parsed)
      const offset = Number(parsed.searchParams.get('offset') ?? 0)
      const isReport = parsed.pathname.endsWith('/v104_iteration_shadow_v5_cycle_reports')
...
test('v104 artifact writer rejects a string cycle before any network write', async () => {
  const requests = []
  const client = createSupabaseIngestionClient({
    url: 'https://example.supabase.co', serviceKey: 'test-only', requireVerifiedStrategy: false,
    fetchImpl: async (...args) => { requests.push(args); return response({ persisted: true, cycle_number: 1, suggestions: 0 }) },
  })
  await assert.rejects(
    client.persistV104IterationShadowArtifacts({ report: { cycleNumber: '1' }, reportSvg: '<svg/>' }),
    /cycleNumber must be a positive safe integer/i,
  )
  assert.equal(requests.length, 0)
})
...
test('v104 settlement uses only its dedicated RPC and preserves PUSH', async () => {
  ...
  await client.settleV104IterationShadowPrediction({
    ...
    strategyVersion: 'v104-seven-head-shadow-v5-best-stage-side-reweight', predictedResult: 'banker', actualResult: 'tie', isHit: null,
    settlementStatus: 'push', settlementFinal: true, settlementSourceAction: '/summary', resolvedAt: '2026-07-21T10:01:00Z',
  })
  assert.match(requests[0].url, /\/rpc\/settle_v104_iteration_shadow_v5_prediction$/)
...
test('v104 network timeout aborts without occupying v102 or v103 queues', { timeout: 250 }, async () => {
  ...
  const client = createSupabaseIngestionClient({
    ...
    fetchImpl: async (url, init) => {
      if (String(url).endsWith('/rpc/issue_v104_iteration_shadow_v5_prediction')) return new Promise((_resolve, reject) => {
