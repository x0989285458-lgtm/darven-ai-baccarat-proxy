...
test('online core client updates app settings through backend-only writer', async () => {
  const calls = []
...
test('online core client toggles feature flags through backend-only writer', async () => {
  const calls = []
  const fetchImpl = async (url, options = {}) => {
    calls.push({ url: String(url), method: options.method ?? 'GET', body: options.body ? JSON.parse(options.body) : null })
    if (String(url).includes('/memory_projects')) return jsonResponse([{ id: 'project-1', slug: 'ai-baccarat' }])
...
test('server accepts admin setting and feature flag updates', async () => {
  const writes = []
