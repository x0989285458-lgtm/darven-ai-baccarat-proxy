...
test('v106 release identity is coherent while the updated capture worker retains protocol v105', () => {
  assert.equal(json('proxy/package.json').version, '1.0.96')
...
test('v106 manifest encodes DB-first through finalize order and exact rollback', () => {
  const manifest = json('release/v106-formal-v10-main-release-manifest.json')
  assert.equal(manifest.applicationVersion, '1.0.96')
  ...
  assert.equal(manifest.gitTag, 'v106.0.0-formal.39')
  ...
  assert.deepEqual(manifest.publicReadinessGate, {
    ...
    intervalMs: 15000,
    requiredIdentity: {
      version: 'v106', buildVersion: 'v106', releaseVersion: 'v106.0.0-formal.39',
      packageVersion: '1.0.96', commit: 'annotated-tag-attested-commit',
    },
