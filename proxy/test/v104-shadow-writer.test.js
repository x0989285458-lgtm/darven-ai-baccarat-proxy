...
test('v104 writer uses only its dedicated issuance RPC and preserves lock diagnostics in immutable payload', async () => {
  ...
  const issued = await client.issueV104ShadowPrediction(candidate)
  assert.match(requests[0].url, /\/rpc\/issue_v104_shadow_prediction$/)
  assert.equal(requests[0].body.p_prediction.strategy_version, 'v104')
...
test('v104 history reads its dedicated view including unsettled issuance rows for restart hydration', async () => {
  const requests = []
  const row = { strategy_version: 'v104', prediction_timing: 'pre_result_context', prediction_issued_at: '2026-07-21T10:00:00Z', settlement_final: false }
  const client = createSupabaseIngestionClient({
    url: 'https://example.supabase.co', serviceKey: 'test-only', requireVerifiedStrategy: false,
    fetchImpl: async (url) => { requests.push(new URL(url)); return response([row]) },
  })
  assert.deepEqual(await client.getV104ShadowHistory(), [row])
  assert.match(requests[0].pathname, /\/rest\/v1\/v104_shadow_history$/)
  assert.equal(requests[0].searchParams.get('strategy_version'), 'eq.v104')
  assert.equal(requests[0].searchParams.get('settlement_final'), null)
  assert.equal(requests[0].searchParams.get('order'), 'prediction_issued_at.desc')
...
test('v104 history GET is aborted at the network layer', { timeout: 250 }, async () => {
  ...
  await assert.rejects(client.getV104ShadowHistory(), /aborted/i)
...
test('v104 issuance fallback GET is aborted at the network layer', { timeout: 250 }, async () => {
  ...
  await assert.rejects(client.readV104ShadowIssuance({ source: 'ofalive99', tableId: 'BAG08', shoe: '104', round: 21 }), /aborted/i)
  assert.equal(aborted, true)
})
...
test('v104 network timeout aborts without occupying v102 or v103 queues', { timeout: 250 }, async () => {
  ...
  const client = createSupabaseIngestionClient({
    ...
    fetchImpl: async (url, init) => {
      if (String(url).endsWith('/rpc/issue_v104_shadow_prediction')) return new Promise((_resolve, reject) => {
  ...
  const hanging = client.issueV104ShadowPrediction(candidate)
