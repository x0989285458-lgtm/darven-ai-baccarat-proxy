...
import { createStableReportSession, FORMAL_MAIN_PREDICTION_WEIGHTS, LEGACY_REPORT_MAIN_WEIGHTS, SIDE_PREDICTION_THRESHOLDS } from '../src/stable-report.js'
...
test('v098 runtime settles each round once with the immutable matching pending prediction', async () => {
  const persisted = []
  const app = createApp({
    autoConnect: false,
    supabaseClient: {
...
test('v098 table snapshots save one pending prediction before the next result arrives', async () => {
  const persisted = []
  const app = createApp({
    autoConnect: false,
    supabaseClient: {
...
test('v098 persistence never creates a prediction result without a matching pending snapshot', async () => {
  const requests = []
...
test('v098 row builder never labels a cross-table prediction as pre-result context', () => {
...
test('v098 stable report settles only the matching prediction saved before reveal', () => {
...
test('v098 stable report shares the approved v097 main weights and side thresholds', () => {
  assert.deepEqual(FORMAL_MAIN_PREDICTION_WEIGHTS, ALL_MT_EQUAL_MAIN_WEIGHTS)
  assert.deepEqual(LEGACY_REPORT_MAIN_WEIGHTS, { shoeRoad: 0.30, askRoad: 0.18, recentTrend: 0.17, bankerPlayerStats: 0.13, auxiliaryRoads: 0.12, beadRoad: 0.10 })
  assert.deepEqual(SIDE_PREDICTION_THRESHOLDS, FORMAL_SIDE_THRESHOLDS)
})

test('v098 analytics trusts saved side actions, excludes ties from main rate, and gates dragon by main direction', async () => {
  const queries = []
...
test('v098 api tables removes actionable prediction after the Render receive-time TTL', async () => {
  let clock = 1_000_000
...
test('v098 streaming request reader aborts as soon as the body exceeds 1MB', async () => {
...
test('v098 member login issues a short-lived opaque bearer session required by table data API', async () => {
  let clock = 2_000_000
  const app = createApp({
    autoConnect: false,
    now: () => clock,
...
test('v098 SSE accepts bearer authorization and rejects every query token', async () => {
  const app = createApp({
    autoConnect: false,
    port: 0,
    memberAuthRequired: true,
...
test('v098 health and status expose one shared build version', async () => {
  ...
  assert.equal(health.version, '098.23')
