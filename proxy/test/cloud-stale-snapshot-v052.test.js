...
test('v052 api/tables does not serve stale Supabase cloud snapshots when live worker state is empty', async () => {
  const app = createApp({
    autoConnect: false,
    deployMode: 'cloud',
...
test('v052 api/status ignores stale cloud snapshot status instead of reporting connected 17 tables', async () => {
  const app = createApp({
    autoConnect: false,
    deployMode: 'cloud',
    captureSource: 'cloud_browser',
