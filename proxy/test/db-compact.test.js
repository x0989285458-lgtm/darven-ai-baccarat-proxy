...
test('persistRound compacts roadmap data but preserves the complete immutable prediction snapshot', async () => {
  const requests = []
  ...
  const atomicBody = requests.find((request) => request.url.includes('/rpc/persist_v104_settled_round')).body
...
test('compact roadmap row never sends null for not-null JSONB remaining counts', () => {
...
test('memory_test_reports row stores report summary and compact metadata only', () => {
...
test('backtest and grid outputs can be stored in tmp/backtests without Supabase', async () => {
...
test('online core routes backtest/grid reports to local tmp files instead of memory_test_reports', async () => {
  const requests = []
...
test('real MT card array is retained and pair flags use rank instead of baccarat point', () => {
...
test('Supabase writer serializes retries and skips duplicate actual rounds in one process', async () => {
  const requests = []
  ...
  const client = createSupabaseIngestionClient({
    ...
    fetchImpl: async (url, init) => {
      ...
      requests.push({ url: String(url), body: JSON.parse(init.body) })
      activeRequests -= 1
      if (failOnce && String(url).includes('/rpc/persist_v104_settled_round')) {
        failOnce = false
  ...
  assert.equal(requests.filter((request) => request.url.includes('/rpc/persist_v104_settled_round')).length, 2)
