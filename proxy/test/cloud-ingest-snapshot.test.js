...
test('same-session concurrent sequences are serialized and never regress or rewrite', async () => {
  ...
  const request = (sequence) => ({
    method: 'POST', url: '/api/cloud-ingest/snapshot', headers: { 'x-worker-key': key },
    body: body({ sequence, snapshot: { buildVersion: '105', connected: true, authenticated: true, sessionId: 'vm-worker', snapshotAt: new Date(now).toISOString(), tables: [{ tableId: 'BAG01', tableType: 'BAC', round: sequence }], rounds: [] } }),
  })
...
test('v102 rank-ledger failure returns 503 without accepting the worker sequence', async () => {
  const app = createTestApp({
...
test('cloud ingest rejects malformed tables and oversized payloads', async () => {
  ...
  const malformed = await app.inject({ method: 'POST', url: '/api/cloud-ingest/snapshot', headers, body: body({ snapshot: { buildVersion: '105', tables: {}, rounds: [] } }) })
