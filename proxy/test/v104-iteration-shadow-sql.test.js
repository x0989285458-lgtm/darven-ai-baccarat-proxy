...
test('v4 migration starts independent zero counters and validates best-observed head weights', () => {
  ...
  assert.match(schemaV4, /from public\.v104_iteration_shadow_v3_runtime_settings[\s\S]{0,220}enabled is distinct from false[\s\S]{0,180}status is distinct from 'shadow_disabled'[\s\S]{0,180}raise exception 'v3 must be fully stopped before v4 activation'/i)
...
test('v5 migration starts independent zero counters and validates approved side reweights', () => {
  ...
  assert.match(schemaV5, /jsonb_typeof\(report_payload->'cycleNumber'\) is distinct from 'number'/i)
  assert.match(schemaV5, /jsonb_typeof\(suggestion->'actionCycle'\) is distinct from 'number'/i)
  assert.match(schemaV5, /mod\(suggested_weight::numeric\*100,5\)<>0/i)
  assert.doesNotMatch(schemaV5, /mod\(round\(suggested_weight::numeric\*100\),5\)/i)
  assert.match(schemaV5, /settled\.resolved_at is distinct from \(p_settlement->>'resolved_at'\)::timestamptz/i)
  assert.match(schemaV5, /existing_suggestion\.suggestion_id is distinct from suggestion->>'id'/i)
  assert.match(schemaV5, /exists \(select 1 from jsonb_each\(suggestion->'suggestedWeights'\)[\s\S]{0,160}jsonb_typeof\(value\) is distinct from 'number'/i)
