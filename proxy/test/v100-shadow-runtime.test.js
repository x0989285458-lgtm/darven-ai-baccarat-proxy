...
import { createV100ShadowRuntime, resolveV100ShadowEnabled } from '../src/v100-shadow-runtime.js'
...
test('v100 shadow runtime is disabled by default and cannot touch writer or formal table', async () => {
  ...
  const runtime = createV100ShadowRuntime({ writer: new Proxy({}, { get() { throw new Error('writer must not be called') } }) })
  const result = await runtime.processSnapshot({ tables: [input], rounds: [round()] })
  assert.deepEqual(result, { enabled: false, shadows: [] })
...
test('enabled v100 runtime rehydrates, applies Final in round order, and scores only from DB ACK', async () => {
  ...
  const input = table()
  const before = structuredClone(input)
  const runtime = createV100ShadowRuntime({ enabled: true, writer, source: 'mt-cloud' })
  const result = await runtime.processSnapshot({ tables: [input], rounds: [round()] })

  assert.deepEqual(calls.map((call) => call[0]), ['read', 'apply'])
  assert.equal(result.enabled, true)
  assert.equal(result.shadows.length, 1)
  assert.equal(result.shadows[0].rankDataAvailable, true)
  assert.equal(result.shadows[0].targetTableId, 'BAG01')
...
test('a restarted v100 runtime reads the exact durable shoe and fails rank closed when progress is stale', async () => {
  ...
  assert.equal(result.shadows[0].rankDataAvailable, false)
  assert.equal(result.shadows[0].side.diagnostics.rank.fallback, 'renormalize')
})
