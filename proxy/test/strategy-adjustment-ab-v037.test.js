...
test('v037 report aggregates strategy adjustment AB hit rates across evaluated rounds', () => {
  ...
  assert.equal(report.strategyAdjustmentSummary.totalEvaluated, 3)
  assert.equal(report.strategyAdjustmentSummary.byMode.normal.evaluated, 3)
  assert.equal(report.strategyAdjustmentSummary.byMode.normal.hits, 0)
  assert.equal(report.strategyAdjustmentSummary.byMode.weakTableDeweight.evaluated, 0)
  assert.equal(report.strategyAdjustmentSummary.byMode.weakTableDeweight.hits, 0)
  assert.equal(report.strategyAdjustmentSummary.byMode.reverseCorrection.evaluated, 0)
  assert.equal(report.strategyAdjustmentSummary.byMode.reverseCorrection.hits, 0)
  assert.equal(report.tables[0].strategyAdjustment.mode, 'reverse-correction')
  assert.equal(report.tables[0].lastConfidence < 50, true)

  const text = formatReportText(report)
  assert.match(text, /策略調整成效/)
  assert.match(text, /反向修正 0%/)
})
