...
test('the same ten-table snapshot and Final use bounded fan-out to V8 alongside V6 and V7', async () => {
  ...
  app.state.setTables(TABLE_IDS.map(table)); await app.waitForServiceWorkIdle()
...
test('V8 issuance and Final errors do not block V6 or V7 fanout', async () => {
  ...
  app.state.setTables([table()]); await app.waitForServiceWorkIdle()
  await app.state.upsertRoundEvent({...table(),round:21,sourceAction:'/show_win',winner:'banker',rawResult:[1,9,2,10,0,0,-1,-1,3,9]})
  await app.waitForServiceWorkIdle()
