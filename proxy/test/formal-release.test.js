...
const latestOnlySql = readFileSync(new URL('../../frontend/supabase/schema_v100_latest_only.sql', import.meta.url), 'utf8')
const rollbackSql = readFileSync(new URL('../../frontend/supabase/rollback_v100_latest_only.sql', import.meta.url), 'utf8')
...
test('v100 formal runtime exposes the exact release identity, approved main weights, and approved side thresholds', async () => {
  ...
  assert.match(latestOnlySql, /v100/)
  assert.match(rollbackSql, /revoke execute on function public\.issue_v100_prediction/i)
