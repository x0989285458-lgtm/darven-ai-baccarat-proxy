...
test('current v106 formal runtime exposes the promoted main weights and inherited side thresholds', async () => {
  ...
  assert.deepEqual(buildFormalActiveStrategy().weights, { ...V106_FORMAL_MAIN_WEIGHTS })
...
test('v104 history derives tie display only from complete persisted v104 evidence and rejects predecessors', async () => {
  ...
  const rows = [
    {
      id: 'tie-action-miss', table_id: 'BAG01', shoe_no: '88', round_no: 13, strategy_version: 'v106',
    ...
    {
      id: 'tie-action', table_id: 'BAG01', shoe_no: '88', round_no: 12, strategy_version: 'v106',
    ...
    {
      id: 'tie-no-action', table_id: 'BAG01', shoe_no: '88', round_no: 11, strategy_version: 'v106',
      predicted_result: 'banker', actual_result: 'tie', is_hit: false, settlement_final: true,
      side_hits: { tie: false }, prediction_features: { prediction_timing: 'pre_result_context', side_actions: { tie: false }, side_hits: { tie: false } },
    },
    {
  ...
  assert.match(decodeURIComponent(requests[0]), /strategy_version=eq\.v106/)
