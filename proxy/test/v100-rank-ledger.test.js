...
test('v102 Supabase client sends only immutable Final evidence and verifies a complete DB-derived ledger ACK', async () => {
  ...
  assert.equal(calls[0].url.endsWith('/rest/v1/rpc/apply_v105_rank_ledger_event'), true)
...
test('v102 migration is conflict-safe and service-only while rollback preserves evidence', () => {
  ...
  assert.match(migration, /create or replace function public\.apply_v105_rank_ledger_event/i)
  ...
  assert.match(migration, /revoke all on function public\.apply_v105_rank_ledger_event/i)
  assert.match(migration, /grant execute on function public\.apply_v105_rank_ledger_event\(jsonb, jsonb\) to service_role/i)
  ...
  assert.match(rollback, /version\s*=\s*'v104'/i)
