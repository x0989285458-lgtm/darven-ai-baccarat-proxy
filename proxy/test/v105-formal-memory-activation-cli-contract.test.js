...
test('memory activation CLI accepts only clean E2E evidence for the exact manifest release', async () => {
  ...
  const files = new Map([
    ...
    ['secret.json', JSON.stringify({ releaseVersion: 'v105.0.0-formal.14', passed: true, verifiedTables: 10, completedAt: '2026-07-24T12:00:00.000Z', token: 'must-not-be-accepted' })],
    ['passed.json', JSON.stringify({ releaseVersion: 'v105.0.0-formal.14', passed: true, verifiedTables: 10, completedAt: '2026-07-24T12:00:00.000Z', finalRows: 917, checks: { proxy: true, database: true, queue: true, cursor: true, frontend: true } })],
  ])
  ...
  assert.equal(strategyWrites[0].releaseVersion, 'v105.0.0-formal.14')
  assert.deepEqual(strategyWrites[0].metrics, { verifiedTables: 10, e2ePassed: true })
  assert.equal(reportWrites[0].releaseVersion, 'v105.0.0-formal.14')
