...
test('v090 cloud capture sends worker admin key header when configured', async () => {
  ...
  const client = createCloudCaptureClient({
    ...
    fetchImpl: async (_url, init = {}) => {
      ...
      return { ok: true, status: 200, json: async () => ({ buildVersion: '100', connected: true, authenticated: true, tables: [] }) }
    },
  })
  await client.tick()
})
...
test('v052 cloud capture clears stale tables when worker loses authenticated table payload', async () => {
  ...
  const client = createCloudCaptureClient({
    ...
    fetchImpl: async () => ({
      ok: true,
      status: 200,
      json: async () => ({ buildVersion: '100', connected: true, authenticated: false, tables: [], errorMessage: 'MT page is open, but no table payload was detected yet.' }),
    }),
  })
