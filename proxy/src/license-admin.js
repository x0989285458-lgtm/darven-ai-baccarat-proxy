...
export function createLicenseAdminClient({ dbConnectionString, pool = null, authPool = null, poolFactory = (options) => new pg.Pool(options) } = {}) {
  ...
  const ownsAuthPool = !authPool && !pool && Boolean(resolvedConnectionString)
  let authDb = authPool ?? (pool ? pool : (resolvedConnectionString ? poolFactory(poolOptions()) : db))
  attachAuthPoolErrorHandler(authDb)

  function attachAuthPoolErrorHandler(target) {
    if (!ownsAuthPool || typeof target?.on !== 'function') return
    target.on('error', () => {})
  }
  ...
  async function authQuery(sql, params) {
    ...
    try {
      ...
      if (target === authDb) {
        const replacement = poolFactory(poolOptions())
        attachAuthPoolErrorHandler(replacement)
        authDb = replacement
  ...
  async function isMaintenanceMode() {
    try {
      const result = await authQuery(`select s.value
  ...
  async function validateAgentLogin({ agentAccount } = {}) {
    ...
    const managerResult = await authQuery("select id, username, role, is_active, created_at from public.manager_accounts where lower(username) = lower($1) and lower(username) = 'dv1788' and is_active = true limit 1", [agentAccount])
    ...
    const result = await authQuery("select id, code, name, role, parent_code, permission, created_at from public.agents where code = $1 and lower(code) <> 'dv1788' and coalesce(is_active, true) = true limit 1", [agentAccount])
...
function isRetryableAuthConnectionError(error) {
  const code = String(error?.code ?? '').toUpperCase()
  if (['ECONNRESET', 'ECONNREFUSED', 'ETIMEDOUT', 'EPIPE', '57P01', '57P02', '57P03'].includes(code) || code.startsWith('08')) return true
  const message = String(error?.message ?? error ?? '')
  return /connection.*(?:terminated|timeout|closed|reset|refused)|timeout.*connect|server closed the connection|socket hang up/i.test(message)
}
