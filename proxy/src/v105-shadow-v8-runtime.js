...
const TABLE_ALLOWLIST = new Set(V105_SHADOW_V8_TABLE_IDS)

export function resolveV105ShadowV8Enabled(env = process.env) {
  return env?.V105_SHADOW_V8_ENABLED !== 'false'
}
...
export function createV105ShadowV8Runtime({
  ...
  async function start() {
    ...
    if (!hydrationPromise) {
      hydrationPromise = Promise.resolve().then(async () => {
        ...
        const rows = await writer.getV105ShadowV8History({ perTableLimit: 60, requestTimeoutMs })
  ...
  async function observeTableNow(table = {}) {
    ...
    if (typeof writer?.issueV105ShadowV8Prediction !== 'function') throw new Error('v105 shadow v8 issuance writer is unavailable')
    const prior = issuanceStreaks.get(String(table.tableId))
    const candidate = buildV105ShadowV8Prediction(table, historyRows, {
      priorShoe: prior?.shoe,
      priorDirection: prior?.direction,
      priorSameSideStreak: prior?.sameSideStreak,
    })
    const operation = Promise.resolve().then(async () => {
      const issued = await withTimeout(writer.issueV105ShadowV8Prediction(candidate), requestTimeoutMs, 'v105 shadow v8 issuance')
  ...
  function settleRound(round = {}) {
    ...
    const operation = Promise.resolve().then(async () => {
      ...
      if (!issued && typeof writer?.readV105ShadowV8Issuance === 'function') {
        issued = await withTimeout(writer.readV105ShadowV8Issuance({
          source: round.source ?? 'ofalive99', tableId: round.tableId, shoe: round.shoe, round: round.round,
        }), requestTimeoutMs, 'v105 shadow v8 issuance read')
      ...
      if (!issued) throw new Error('v105 shadow v8 settlement has no immutable issuance')
      if (typeof writer?.settleV105ShadowV8Prediction !== 'function') throw new Error('v105 shadow v8 settlement writer is unavailable')
      const settlement = buildV105ShadowV8Settlement(round, issued)
      const result = await withTimeout(writer.settleV105ShadowV8Prediction(settlement), requestTimeoutMs, 'v105 shadow v8 settlement')
      if (String(result?.predictionId ?? result?.prediction_id ?? '') !== String(issued.predictionId)) throw new Error('v105 shadow v8 settlement acknowledgement failed')
  ...
  return {
    ...
    snapshot: () => ({
      ...
      status,
      error,
      historySource: 'v105_shadow_v8_only',
...
function isOwnHistoryRow(row) {
  return (row?.strategy_version ?? row?.strategyVersion) === V105_SHADOW_V8_VERSION
...
function isValidIssued(issued) {
  return Boolean(issued?.predictionId && issued?.issuedAt && issued?.strategyVersion === V105_SHADOW_V8_VERSION && issued?.predictionTiming === 'pre_result_context')
}
