...
const mode = String(process.env.MAIN54_BATCH100_TEST_MODE ?? '')
if (mode !== 'rollback-only' || process.env.MAIN54_BATCH100_ROLLBACK_CONFIRM !== 'ROLLBACK_ONLY_NO_COMMIT') {
  throw new Error('Main54 batch100 test requires rollback-only and ROLLBACK_ONLY_NO_COMMIT')
}
...
const migrationPath = new URL('../supabase/migrations/20260828010000_v105_capture_outbox_batch100_contract.sql', import.meta.url)
...
async function insertHundred(sessionId) {
...
try {
  ...
  await insertHundred(`${identity}-complete`)
  const claimedForComplete = await db.query('select session_id,sequence,claim_token,attempts,status from public.claim_v105_capture_settlement_outbox_batch(100)')
  if (claimedForComplete.rows.length !== 100 || claimedForComplete.rows.some((row) => row.status !== 'processing' || row.session_id !== `${identity}-complete`)) throw new Error('Main54 DB did not claim exactly 100 exact-session leases')
  ...
  if (completed.rows[0]?.ack?.completed !== true || Number(completed.rows[0]?.ack?.count) !== 100) throw new Error('Main54 DB did not complete exactly 100 leases')

  await insertHundred(`${identity}-fail`)
  const claimedForFail = await db.query('select session_id,sequence,claim_token,attempts,status from public.claim_v105_capture_settlement_outbox_batch(100)')
  if (claimedForFail.rows.length !== 100 || claimedForFail.rows.some((row) => row.status !== 'processing' || row.session_id !== `${identity}-fail`)) {
    const sessions = [...new Set(claimedForFail.rows.map((row) => row.session_id))]
    throw new Error(`Main54 DB did not claim second exact-session 100-lease batch: count=${claimedForFail.rows.length} sessions=${sessions.join(',')}`)
  }
  ...
  if (failed.rows[0]?.ack?.failed !== true || Number(failed.rows[0]?.ack?.count) !== 100) throw new Error('Main54 DB did not fail exactly 100 leases')
  ...
  const hundredOne = Array.from({ length: 101 }, (_, index) => ({ session_id: `${identity}-invalid`, sequence: index + 1, claim_token: '00000000-0000-0000-0000-000000000001', attempt: 1 }))
  let complete101Rejected = false
  await db.query('savepoint complete101')
  try { await db.query('select public.complete_v105_capture_settlement_outbox_batch($1::jsonb)', [JSON.stringify(hundredOne)]) }
  catch (error) { complete101Rejected = /batch claim identity is invalid/i.test(String(error?.message)); await db.query('rollback to savepoint complete101') }
  if (!complete101Rejected) throw new Error('Main54 DB accepted 101 completion claims')
  let fail101Rejected = false
  await db.query('savepoint fail101')
  try { await db.query('select public.fail_v105_capture_settlement_outbox_batch($1::jsonb,$2)', [JSON.stringify(hundredOne), 'invalid']) }
  catch (error) { fail101Rejected = /batch claim identity is invalid/i.test(String(error?.message)); await db.query('rollback to savepoint fail101') }
  if (!fail101Rejected) throw new Error('Main54 DB accepted 101 failure claims')
  ...
  if (definitions.rows.length !== 3 || !definitions.rows.every((row) => row.definition.includes('100'))) throw new Error('Main54 installed definitions do not bind batch100')
  await db.query('rollback'); rolledBack = true
  console.log(JSON.stringify({ ok: true, exactSessionClaims: true, claimed: 100, completed: 100, failed: 100, poisonIsolated: true, rejected101: true, rollback: true }))
} finally {
