...
const EXPECTED_EXEC_START = '/usr/bin/docker run --name darven-worker --network host --shm-size=1g --env-file /etc/darven-worker/worker.env -v /var/lib/darven-worker:/app/data --entrypoint /bin/sh ${WORKER_IMAGE} -c "Xvfb :99 -screen 0 1440x1000x24 -nolisten tcp >/tmp/xvfb.log 2>&1 & sleep 2; export DISPLAY=:99; exec npm start"'
...
test('v101 worker image and systemd deployment fail closed and preserve durable queue state', async () => {
  ...
  assert.match(dockerfile, /org\.opencontainers\.image\.version="v101"/)
  ...
  assert.doesNotMatch(values.ExecStart, /(?:^|\s)-d(?:\s|$)|:ro(?:\s|$)|\/bin\/false/)
