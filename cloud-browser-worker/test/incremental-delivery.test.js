...
test('durable FIFO retains a failed head while collecting the next round', async (t) => {
...
test('only final summary queues observes and ACKs an identity first seen as exact show_poker', async (t) => {
...
test('retains every recognized MT card-result action in the durable round buffer', () => {
...
test('excludes card-shaped payloads without a recognized MT round action', () => {
  const payload = JSON.stringify({
