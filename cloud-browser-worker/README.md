# Darven Cloud Browser Worker v102
...
- 正式Build：`101`
- Push Protocol：`v102`
- Image：每個Release使用不可變Tag `darven-worker:v102-<sha7>`
...
5. `/health`回`buildVersion: "102"`。
6. `/snapshot`只在帶`x-worker-admin-key`時可讀。
7. Proxy `/api/status`為`buildVersion: "v102"`、10桌、Persistence正常。
