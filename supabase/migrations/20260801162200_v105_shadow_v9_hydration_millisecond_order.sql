-- Align v9 compact hydration ordering with Node.js Date millisecond precision.
-- Additive/repeatable: replaces one read-only SECURITY DEFINER RPC; preserves all evidence.
begin;
...
revoke all on function public.get_v105_shadow_v9_compact_history(integer) from public;
revoke all on function public.get_v105_shadow_v9_compact_history(integer) from anon, authenticated, service_role;
...
begin
  ...
  where function_row.oid = 'public.get_v105_shadow_v9_compact_history(integer)'::pg_catalog.regprocedure
  ...
  if unexpected_roles is not null then
    raise exception 'unexpected execute ACL on v105 shadow v9 compact history: %', unexpected_roles
      using errcode = '42501';
  end if;
end;
$acl$;
