-- v106 DB-first additive formal contract. This migration does not activate v106.
-- Existing v105 and V10 shadow evidence and predecessor writer grants are retained.

create unique index if not exists ai_strategy_versions_exactly_one_active
  on public.ai_strategy_versions ((status)) where status = 'active';

-- Harden the predecessor issuance against an activation race. The shared row lock is
-- held until the issuance transaction finishes; activation takes the matching update lock.
...
begin
  ...
  for share;
  if not found then
    raise exception 'v106 Active strategy verification failed';
  end if;
...
create or replace function public.settle_v106_prediction(p_roadmap jsonb, p_settlement jsonb)
returns jsonb
language plpgsql
security definer
set search_path = public
...
begin
  ...
  if existing.id is null
     or existing.strategy_version is distinct from 'v106'
     ...
     or p_settlement->>'strategy_version' is distinct from 'v106'
    ...
    raise exception 'v106 settlement identity mismatch';
  end if;

  if existing.prediction_issued_at is null or existing.issued_prediction_payload is null then
    raise exception 'v106 prediction has no immutable pre-result evidence';
  end if;
  ...
  if p_settlement->>'actual_result' not in ('banker', 'player', 'tie')
     ...
     or p_settlement->>'settlement_source_action' is distinct from '/summary'
     or coalesce((p_settlement->>'settlement_final')::boolean, false) is not true
     or (p_settlement->>'is_hit')::boolean is distinct from (
       case when p_settlement->>'actual_result' = 'tie' then false
            else p_settlement->>'actual_result' = existing.predicted_result end
     ) then
    raise exception 'v106 settlement requires verified Final source_action /summary and consistent is_hit';
  end if;
  ...
  if existing.settlement_final is true then
    if existing.actual_result is distinct from p_settlement->>'actual_result'
       ...
       or existing.settlement_source_action is distinct from '/summary'
      ...
      raise exception 'conflicting v106 Final settlement';
    end if;
  ...
  insert into public.daily_roadmap_events (
    ...
    p_roadmap->>'source', p_roadmap->>'table_id', p_roadmap->>'shoe_no', (p_roadmap->>'round_no')::integer,
    p_roadmap->>'main_result', nullif(p_roadmap->>'banker_points', '')::integer,
    nullif(p_roadmap->>'player_points', '')::integer,
    coalesce((p_roadmap->>'banker_pair')::boolean, false),
    coalesce((p_roadmap->>'player_pair')::boolean, false),
    coalesce((p_roadmap->>'super_six')::boolean, false),
    coalesce((p_roadmap->>'banker_dragon')::boolean, false),
    coalesce((p_roadmap->>'player_dragon')::boolean, false),
    p_roadmap->>'bead_code', coalesce(p_roadmap->'raw_event', '{}'::jsonb),
    coalesce(p_roadmap->'player_card_codes', '[]'::jsonb),
    coalesce(p_roadmap->'banker_card_codes', '[]'::jsonb),
    coalesce(p_roadmap->'player_card_points', '[]'::jsonb),
    coalesce(p_roadmap->'banker_card_points', '[]'::jsonb),
    coalesce(p_roadmap->'player_card_ranks', '[]'::jsonb),
    coalesce(p_roadmap->'banker_card_ranks', '[]'::jsonb),
    coalesce(p_roadmap->'player_card_faces', '[]'::jsonb),
    coalesce(p_roadmap->'banker_card_faces', '[]'::jsonb),
    coalesce((p_roadmap->>'player_drew')::boolean, false),
    coalesce((p_roadmap->>'banker_drew')::boolean, false),
    coalesce((p_roadmap->>'player_natural')::boolean, false),
    coalesce((p_roadmap->>'banker_natural')::boolean, false),
    coalesce(p_roadmap->'remaining_rank_counts', '{}'::jsonb),
    coalesce(p_roadmap->'remaining_point_counts', '{}'::jsonb)
  )
  ...
  if already_settled then
    return jsonb_build_object('persisted', true, 'predictionDurable', true, 'roadmapDurable', true,
      'prediction_id', existing.id, 'duplicate', true);
  end if;
  ...
  set actual_result = p_settlement->>'actual_result',
      ...
      settlement_source_action = '/summary',
  ...
  return jsonb_build_object('persisted', true, 'predictionDurable', true, 'roadmapDurable', true,
    'prediction_id', existing.id, 'duplicate', false);
end;
$$;

create or replace function public.reconcile_v106_prediction_lifecycle(
  p_source text, p_table_id text, p_current_shoe text, p_current_visible_round integer
)
returns jsonb
language plpgsql
security definer
...
begin
  if nullif(p_source, '') is null or nullif(p_table_id, '') is null
     or nullif(p_current_shoe, '') is null or p_current_visible_round is null
     or p_current_visible_round < 1 then
    raise exception 'v106 lifecycle identity is incomplete';
  end if;
  ...
  with classified as (
    ...
    set issuance_status = case
        ...
        issuance_status_reason = case
          ...
          else 'live_screen_identity_current_or_future' end
    where strategy_version = 'v106' and source = p_source and table_id = p_table_id
      and prediction_issued_at is not null and settlement_final is not true
  ...
  return jsonb_build_object('source', p_source, 'table_id', p_table_id,
    'current_shoe', p_current_shoe, 'current_visible_round', p_current_visible_round,
    'pending', pending_count, 'expired_no_final', expired_count,
    'abandoned_shoe_change', abandoned_count, 'updated_total', updated_count);
end;
$$;

create or replace function public.get_v106_prediction_lifecycle_stats()
returns jsonb
language sql
stable
security definer
...
as $$
  ...
  from public.daily_prediction_results
  where strategy_version = 'v106' and prediction_issued_at is not null;
$$;
...
select
  'v106', 'archived', learned_from_date, 0, total_hit_rate, high_confidence_hit_rate,
  jsonb_build_object(
    'v7RoadCycle', 0.315, 'v8AskRoad', 0.315,
    'recentPracticalCalibration', 0.18, 'shoeBankerPlayerBias', 0.09,
    'uncommonRoadStructure', 0.10
  ),
  coalesce(metrics, '{}'::jsonb) || jsonb_build_object(
    'mode', 'formal_v10_eligible_main_v105_fallback_and_sides',
    'auto_adjust', false,
    'main_strategy', 'v106_V10大路罕見結構正式版',
    'side_strategy', 'v106_副預測逐位元沿用v105正式規則',
    'activation_gate', 'structureDiagnostics.eligible === true',
    'fallback', 'exact formal v105 main projection'
  ),
  'DB-first staged v106; activate only with reviewed activation operation.',
  now(), null
from public.ai_strategy_versions where version = 'v105'
on conflict (version) do nothing;

revoke all on function public.issue_v106_prediction(jsonb) from public, anon, authenticated;
revoke all on function public.settle_v106_prediction(jsonb, jsonb) from public, anon, authenticated;
revoke all on function public.reconcile_v106_prediction_lifecycle(text, text, text, integer) from public, anon, authenticated;
revoke all on function public.get_v106_prediction_lifecycle_stats() from public, anon, authenticated;
...
-- Explicitly retain the predecessor settlement/reconciliation surface during DB-first staging.
-- Do not re-grant issue_v105_prediction here: a safe idempotent migration rerun must
-- never undo fence_v105_new_issuance.sql.
