...
begin
  ...
  if expected < 1 or expected > 100 or affected <> expected then
...
do $$
declare claim_def text; complete_def text; fail_def text;
begin
  select pg_catalog.regexp_replace(pg_catalog.lower(pg_catalog.pg_get_functiondef('public.claim_v105_capture_settlement_outbox_batch(integer)'::pg_catalog.regprocedure)), '[[:space:]]+', '', 'g') into claim_def;
  select pg_catalog.regexp_replace(pg_catalog.lower(pg_catalog.pg_get_functiondef('public.complete_v105_capture_settlement_outbox_batch(jsonb)'::pg_catalog.regprocedure)), '[[:space:]]+', '', 'g') into complete_def;
  select pg_catalog.regexp_replace(pg_catalog.lower(pg_catalog.pg_get_functiondef('public.fail_v105_capture_settlement_outbox_batch(jsonb,text)'::pg_catalog.regprocedure)), '[[:space:]]+', '', 'g') into fail_def;
  if pg_catalog.strpos(claim_def, 'limitgreatest(1,least(coalesce(p_limit,10),100))') = 0 then
    raise exception 'capture outbox batch100 claim contract verification failed';
  end if;
  if pg_catalog.strpos(complete_def, 'expected>100') = 0 then
    raise exception 'capture outbox batch100 completion contract verification failed';
  end if;
  if pg_catalog.strpos(fail_def, 'expected>100') = 0 then
    raise exception 'capture outbox batch100 failure contract verification failed';
  end if;
end;
$$;
