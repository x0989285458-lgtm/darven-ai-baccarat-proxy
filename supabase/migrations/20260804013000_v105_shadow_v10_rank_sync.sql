-- v105-shadow-v10-big-road-uncommon-structure-rank-synchronized: backend-only ask-road shadow persistence, isolated from V5/V6 and every prior shadow version.
begin;

create table if not exists public.v105_shadow_v10_rank_sync_runtime_settings (
  release_candidate text primary key,
  strategy_version text not null check (strategy_version = 'v105-shadow-v10-big-road-uncommon-structure-rank-synchronized'),
...
create table if not exists public.v105_shadow_v10_rank_sync_sequence_counters (
  release_candidate text primary key,
...
create table if not exists public.v105_shadow_v10_rank_sync_issuances (
  ...
  constraint v105_shadow_v10_rank_sync_issuance_identity_unique unique (source,table_id,shoe_no,round_no,strategy_version)
);

create table if not exists public.v105_shadow_v10_rank_sync_settlements (
  id uuid primary key default gen_random_uuid(),
  ...
  prediction_id uuid not null unique references public.v105_shadow_v10_rank_sync_issuances(id),
  source text not null,
  table_id text not null check (table_id in ('BAG01','BAG02','BAG03','BAG03A','BAG05','BAG06','BAG07','BAG08','BAG09','BAG10')),
  shoe_no text not null,
  round_no integer not null check (round_no > 0),
  ...
  created_at timestamptz not null default now(),
  constraint v105_shadow_v10_rank_sync_main_semantics check (
...
create index if not exists v105_shadow_v10_rank_sync_issuances_issued_idx
  on public.v105_shadow_v10_rank_sync_issuances (prediction_issued_at desc,id desc);
create index if not exists v105_shadow_v10_rank_sync_settlements_resolved_idx
  on public.v105_shadow_v10_rank_sync_settlements (resolved_at desc,prediction_id);

alter table public.v105_shadow_v10_rank_sync_runtime_settings enable row level security;
alter table public.v105_shadow_v10_rank_sync_sequence_counters enable row level security;
alter table public.v105_shadow_v10_rank_sync_issuances enable row level security;
alter table public.v105_shadow_v10_rank_sync_settlements enable row level security;

revoke all on table public.v105_shadow_v10_rank_sync_runtime_settings from public,anon,authenticated,service_role;
revoke all on table public.v105_shadow_v10_rank_sync_sequence_counters from public,anon,authenticated,service_role;
revoke all on table public.v105_shadow_v10_rank_sync_issuances from public,anon,authenticated,service_role;
revoke all on table public.v105_shadow_v10_rank_sync_settlements from public,anon,authenticated,service_role;
grant select on table public.v105_shadow_v10_rank_sync_runtime_settings to service_role;
grant select on table public.v105_shadow_v10_rank_sync_sequence_counters to service_role;
grant select on table public.v105_shadow_v10_rank_sync_issuances to service_role;
grant select on table public.v105_shadow_v10_rank_sync_settlements to service_role;

insert into public.v105_shadow_v10_rank_sync_runtime_settings (
  release_candidate,strategy_version,status,enabled,active_strategy_version,updated_at
) values ('v105-shadow-v10-big-road-uncommon-structure-rank-synchronized','v105-shadow-v10-big-road-uncommon-structure-rank-synchronized','shadow',true,'v105',now())
on conflict (release_candidate) do nothing;

insert into public.v105_shadow_v10_rank_sync_sequence_counters (release_candidate,settlement_count)
values ('v105-shadow-v10-big-road-uncommon-structure-rank-synchronized',0)
on conflict (release_candidate) do nothing;
...
create or replace function public.settle_v105_shadow_v10_rank_sync_prediction(p_settlement jsonb)
returns jsonb
language plpgsql
security definer
set search_path=pg_catalog
...
declare
  issued public.v105_shadow_v10_rank_sync_issuances%rowtype;
  existing public.v105_shadow_v10_rank_sync_settlements%rowtype;
  counters public.v105_shadow_v10_rank_sync_sequence_counters%rowtype;
...
begin
  if p_settlement->>'strategy_version' is distinct from 'v105-shadow-v10-big-road-uncommon-structure-rank-synchronized'
    ...
    raise exception 'v105-shadow-v10-big-road-uncommon-structure-rank-synchronized settlement payload is invalid';
  end if;
  select * into issued from public.v105_shadow_v10_rank_sync_issuances
    where id=(p_settlement->>'prediction_id')::uuid for update;
  if not found
     or issued.strategy_version <> 'v105-shadow-v10-big-road-uncommon-structure-rank-synchronized'
    ...
    raise exception 'v105-shadow-v10-big-road-uncommon-structure-rank-synchronized settlement identity mismatch';
  end if;
  ...
  select * into existing from public.v105_shadow_v10_rank_sync_settlements where prediction_id=issued.id;
  if found then
    if existing.actual_result is distinct from p_settlement->>'actual_result'
      ...
      raise exception 'conflicting v105-shadow-v10-big-road-uncommon-structure-rank-synchronized settlement';
    end if;
  ...
  select * into counters from public.v105_shadow_v10_rank_sync_sequence_counters
    where release_candidate='v105-shadow-v10-big-road-uncommon-structure-rank-synchronized' for update;
  if not found then raise exception 'v105-shadow-v10-big-road-uncommon-structure-rank-synchronized sequence counter is unavailable'; end if;
  next_sequence := counters.settlement_count + 1;
  insert into public.v105_shadow_v10_rank_sync_settlements (
  ...
  update public.v105_shadow_v10_rank_sync_sequence_counters
...
create or replace view public.v105_shadow_v10_rank_sync_history
...
from public.v105_shadow_v10_rank_sync_issuances i
left join public.v105_shadow_v10_rank_sync_settlements s on s.prediction_id=i.id;

create or replace function public.get_v105_shadow_v10_rank_sync_compact_history(p_per_table_limit integer)
returns table (
  prediction_id uuid,
  source text,
  table_id text,
...
begin
  ...
  with target_tables(target_table_id) as (
    ...
    cross join lateral (
      ...
      from public.v105_shadow_v10_rank_sync_issuances i
      inner join public.v105_shadow_v10_rank_sync_settlements s on s.prediction_id=i.id
      where i.table_id=t.target_table_id
        and i.source='ofalive99'
        and i.strategy_version='v105-shadow-v10-big-road-uncommon-structure-rank-synchronized'
    ...
    cross join lateral (
      ...
      where i.table_id=t.target_table_id
        ...
        and i.prediction_timing='pre_result_context'
        and not exists (
          select 1 from public.v105_shadow_v10_rank_sync_settlements final_settlement
...
revoke all on function public.issue_v105_shadow_v10_rank_sync_prediction(jsonb) from public,anon,authenticated,service_role;
revoke all on function public.settle_v105_shadow_v10_rank_sync_prediction(jsonb) from public,anon,authenticated,service_role;
revoke all on function public.get_v105_shadow_v10_rank_sync_compact_history(integer) from public,anon,authenticated,service_role;
grant execute on function public.issue_v105_shadow_v10_rank_sync_prediction(jsonb) to service_role;
grant execute on function public.settle_v105_shadow_v10_rank_sync_prediction(jsonb) to service_role;
grant execute on function public.get_v105_shadow_v10_rank_sync_compact_history(integer) to service_role;
revoke all on public.v105_shadow_v10_rank_sync_history from public,anon,authenticated,service_role;
grant select on public.v105_shadow_v10_rank_sync_history to service_role;
