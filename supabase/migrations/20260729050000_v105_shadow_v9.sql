-- v105-shadow-v9-weighted-v7-v8: backend-only ask-road shadow persistence, isolated from V5/V6 and every prior shadow version.
begin;

create table if not exists public.v105_shadow_v9_runtime_settings (
  release_candidate text primary key,
  strategy_version text not null check (strategy_version = 'v105-shadow-v9-weighted-v7-v8'),
...
create table if not exists public.v105_shadow_v9_sequence_counters (
  release_candidate text primary key,
...
create table if not exists public.v105_shadow_v9_issuances (
  ...
  constraint v105_shadow_v9_issuance_identity_unique unique (source,table_id,shoe_no,round_no,strategy_version)
);

create table if not exists public.v105_shadow_v9_settlements (
  id uuid primary key default gen_random_uuid(),
  ...
  prediction_id uuid not null unique references public.v105_shadow_v9_issuances(id),
  source text not null,
  table_id text not null check (table_id in ('BAG01','BAG02','BAG03','BAG03A','BAG05','BAG06','BAG07','BAG08','BAG09','BAG10')),
  shoe_no text not null,
  round_no integer not null check (round_no > 0),
  ...
  created_at timestamptz not null default now(),
  constraint v105_shadow_v9_main_semantics check (
...
create index if not exists v105_shadow_v9_issuances_issued_idx
  on public.v105_shadow_v9_issuances (prediction_issued_at desc,id desc);
create index if not exists v105_shadow_v9_settlements_resolved_idx
  on public.v105_shadow_v9_settlements (resolved_at desc,prediction_id);

alter table public.v105_shadow_v9_runtime_settings enable row level security;
alter table public.v105_shadow_v9_sequence_counters enable row level security;
alter table public.v105_shadow_v9_issuances enable row level security;
alter table public.v105_shadow_v9_settlements enable row level security;

revoke all on table public.v105_shadow_v9_runtime_settings from public,anon,authenticated,service_role;
revoke all on table public.v105_shadow_v9_sequence_counters from public,anon,authenticated,service_role;
revoke all on table public.v105_shadow_v9_issuances from public,anon,authenticated,service_role;
revoke all on table public.v105_shadow_v9_settlements from public,anon,authenticated,service_role;
grant select on table public.v105_shadow_v9_runtime_settings to service_role;
grant select on table public.v105_shadow_v9_sequence_counters to service_role;
grant select on table public.v105_shadow_v9_issuances to service_role;
grant select on table public.v105_shadow_v9_settlements to service_role;

insert into public.v105_shadow_v9_runtime_settings (
  release_candidate,strategy_version,status,enabled,active_strategy_version,updated_at
) values ('v105-shadow-v9-weighted-v7-v8','v105-shadow-v9-weighted-v7-v8','shadow',true,'v105',now())
on conflict (release_candidate) do nothing;

insert into public.v105_shadow_v9_sequence_counters (release_candidate,settlement_count)
values ('v105-shadow-v9-weighted-v7-v8',0)
on conflict (release_candidate) do nothing;

create or replace function public.issue_v105_shadow_v9_prediction(p_prediction jsonb)
returns jsonb
language plpgsql
security definer
...
as $$
declare
  issued public.v105_shadow_v9_issuances%rowtype;
begin
  ...
  perform 1 from public.v105_shadow_v9_runtime_settings
    where release_candidate='v105-shadow-v9-weighted-v7-v8' and strategy_version='v105-shadow-v9-weighted-v7-v8'
      and status='shadow' and enabled=true and active_strategy_version='v105'
    for share;
  if not found then raise exception 'v105-shadow-v9-weighted-v7-v8 is disabled'; end if;
  ...
  if nullif(p_prediction->>'source','') is null
     ...
     or p_prediction->>'strategy_version' is distinct from 'v105-shadow-v9-weighted-v7-v8'
     ...
     or p_prediction->'prediction_payload'->>'strategyVersion' is distinct from 'v105-shadow-v9-weighted-v7-v8'
     or p_prediction->'prediction_payload'->>'releaseCandidate' is distinct from 'v105-shadow-v9-weighted-v7-v8'
     ...
     or p_prediction->'prediction_payload'->'featureWeights' is distinct from
       '{"v7RoadCycle":0.35,"v8AskRoad":0.35,"recentPracticalCalibration":0.20,"shoeBankerPlayerBias":0.10}'::jsonb
     or jsonb_typeof(p_prediction->'prediction_payload'->'signals') is distinct from 'object'
     or (select count(*) from jsonb_object_keys(p_prediction->'prediction_payload'->'signals')) <> 4
     ...
     or (select count(*) from jsonb_object_keys(p_prediction->'prediction_payload'->'scoreSources')) <> 4
    ...
    raise exception 'v105-shadow-v9-weighted-v7-v8 issuance payload is invalid';
  end if;

  insert into public.v105_shadow_v9_issuances (
  ...
  select * into issued from public.v105_shadow_v9_issuances
  ...
  if not found or issued.prediction_payload is distinct from p_prediction->'prediction_payload' then
    raise exception 'conflicting v105-shadow-v9-weighted-v7-v8 issuance';
  end if;
...
create or replace function public.settle_v105_shadow_v9_prediction(p_settlement jsonb)
returns jsonb
language plpgsql
security definer
set search_path=pg_catalog,public,extensions
...
declare
  issued public.v105_shadow_v9_issuances%rowtype;
  existing public.v105_shadow_v9_settlements%rowtype;
  counters public.v105_shadow_v9_sequence_counters%rowtype;
...
begin
  if p_settlement->>'strategy_version' is distinct from 'v105-shadow-v9-weighted-v7-v8'
    ...
    raise exception 'v105-shadow-v9-weighted-v7-v8 settlement payload is invalid';
  end if;
  select * into issued from public.v105_shadow_v9_issuances
    where id=(p_settlement->>'prediction_id')::uuid for update;
  if not found
     or issued.strategy_version <> 'v105-shadow-v9-weighted-v7-v8'
    ...
    raise exception 'v105-shadow-v9-weighted-v7-v8 settlement identity mismatch';
  end if;
  ...
  if jsonb_typeof(p_settlement->'actual_facts'->'bankerCardRanks') is distinct from 'array'
    ...
    raise exception 'V9 exact card ranks are required';
  end if;
  if jsonb_array_length(p_settlement->'actual_facts'->'bankerCardRanks') not between 2 and 3
    ...
    raise exception 'V9 exact card ranks are invalid';
  end if;
  if (p_settlement->'actual_facts'->>'bankerPoints')::integer is distinct from
    ...
    raise exception 'V9 actual facts do not match exact cards';
  end if;
  if (p_settlement->'actual_facts'->>'tie')::boolean is distinct from (p_settlement->>'actual_result'='tie')
    ...
    raise exception 'V9 actual facts are inconsistent';
  end if;
  ...
  if p_settlement->>'settlement_status' is distinct from expected
    ...
    raise exception 'V9 main outcome is inconsistent';
  end if;
  ...
  foreach head_key in array array['main','tie','superSix','bankerDragon','playerDragon','bankerPair','playerPair'] loop
    ...
    if jsonb_typeof(head) is distinct from 'object' or (head->>'action')::boolean is distinct from issued_action then
      raise exception 'V9 head action mismatch: %',head_key;
    end if;
    if not issued_action then
      if head->>'status' is distinct from 'no_action' or head->'isHit' is distinct from 'null'::jsonb
        ...
        raise exception 'V9 no-action result mismatch: %',head_key;
      end if;
      continue;
    end if;
    ...
    if head->>'status' is distinct from expected_head_status
      ...
      raise exception 'V9 head outcome mismatch: %',head_key;
    end if;
    ...
    if fixed_stake is distinct from 1::numeric or weighted_stake is distinct from issued_units
      ...
      raise exception 'V9 payout mismatch: %',head_key;
    end if;
  end loop;

  select * into existing from public.v105_shadow_v9_settlements where prediction_id=issued.id;
  if found then
    if existing.actual_result is distinct from p_settlement->>'actual_result'
      ...
      raise exception 'conflicting v105-shadow-v9-weighted-v7-v8 settlement';
    end if;
  ...
  select * into counters from public.v105_shadow_v9_sequence_counters
    where release_candidate='v105-shadow-v9-weighted-v7-v8' for update;
  if not found then raise exception 'v105-shadow-v9-weighted-v7-v8 sequence counter is unavailable'; end if;
  next_sequence := counters.settlement_count + 1;
  insert into public.v105_shadow_v9_settlements (
  ...
  update public.v105_shadow_v9_sequence_counters
...
create or replace view public.v105_shadow_v9_history
...
revoke all on function public.issue_v105_shadow_v9_prediction(jsonb) from public,anon,authenticated,service_role;
revoke all on function public.settle_v105_shadow_v9_prediction(jsonb) from public,anon,authenticated,service_role;
grant execute on function public.issue_v105_shadow_v9_prediction(jsonb) to service_role;
grant execute on function public.settle_v105_shadow_v9_prediction(jsonb) to service_role;
revoke all on public.v105_shadow_v9_history from public,anon,authenticated,service_role;
grant select on public.v105_shadow_v9_history to service_role;
