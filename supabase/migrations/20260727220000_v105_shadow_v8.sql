-- v105-shadow-v8-run-length-ask-road: backend-only ask-road shadow persistence, isolated from V5/V6 and every prior shadow version.
begin;

create table if not exists public.v105_shadow_v8_runtime_settings (
  release_candidate text primary key,
  strategy_version text not null check (strategy_version = 'v105-shadow-v8-run-length-ask-road'),
...
create table if not exists public.v105_shadow_v8_sequence_counters (
  release_candidate text primary key,
...
create table if not exists public.v105_shadow_v8_issuances (
  ...
  constraint v105_shadow_v8_issuance_identity_unique unique (source,table_id,shoe_no,round_no,strategy_version)
);

create table if not exists public.v105_shadow_v8_settlements (
  id uuid primary key default gen_random_uuid(),
  ...
  prediction_id uuid not null unique references public.v105_shadow_v8_issuances(id),
  source text not null,
  table_id text not null check (table_id in ('BAG01','BAG02','BAG03','BAG03A','BAG05','BAG06','BAG07','BAG08','BAG09','BAG10')),
  shoe_no text not null,
  round_no integer not null check (round_no > 0),
  ...
  created_at timestamptz not null default now(),
  constraint v105_shadow_v8_main_semantics check (
...
create index if not exists v105_shadow_v8_issuances_issued_idx
  on public.v105_shadow_v8_issuances (prediction_issued_at desc,id desc);
create index if not exists v105_shadow_v8_settlements_resolved_idx
  on public.v105_shadow_v8_settlements (resolved_at desc,prediction_id);

alter table public.v105_shadow_v8_runtime_settings enable row level security;
alter table public.v105_shadow_v8_sequence_counters enable row level security;
alter table public.v105_shadow_v8_issuances enable row level security;
alter table public.v105_shadow_v8_settlements enable row level security;

revoke all on table public.v105_shadow_v8_runtime_settings from public,anon,authenticated,service_role;
revoke all on table public.v105_shadow_v8_sequence_counters from public,anon,authenticated,service_role;
revoke all on table public.v105_shadow_v8_issuances from public,anon,authenticated,service_role;
revoke all on table public.v105_shadow_v8_settlements from public,anon,authenticated,service_role;
grant select on table public.v105_shadow_v8_runtime_settings to service_role;
grant select on table public.v105_shadow_v8_sequence_counters to service_role;
grant select on table public.v105_shadow_v8_issuances to service_role;
grant select on table public.v105_shadow_v8_settlements to service_role;

insert into public.v105_shadow_v8_runtime_settings (
  release_candidate,strategy_version,status,enabled,active_strategy_version,updated_at
) values ('v105-shadow-v8-run-length-ask-road','v105-shadow-v8-run-length-ask-road','shadow',true,'v105',now())
on conflict (release_candidate) do nothing;

insert into public.v105_shadow_v8_sequence_counters (release_candidate,settlement_count)
values ('v105-shadow-v8-run-length-ask-road',0)
on conflict (release_candidate) do nothing;

create or replace function public.issue_v105_shadow_v8_prediction(p_prediction jsonb)
returns jsonb
language plpgsql
security definer
...
as $$
declare
  issued public.v105_shadow_v8_issuances%rowtype;
begin
  ...
  perform 1 from public.v105_shadow_v8_runtime_settings
    where release_candidate='v105-shadow-v8-run-length-ask-road' and strategy_version='v105-shadow-v8-run-length-ask-road'
      and status='shadow' and enabled=true and active_strategy_version='v105'
    for share;
  if not found then raise exception 'v105-shadow-v8-run-length-ask-road is disabled'; end if;
  ...
  if nullif(p_prediction->>'source','') is null
     ...
     or p_prediction->>'strategy_version' is distinct from 'v105-shadow-v8-run-length-ask-road'
     ...
     or p_prediction->'prediction_payload'->>'strategyVersion' is distinct from 'v105-shadow-v8-run-length-ask-road'
     or p_prediction->'prediction_payload'->>'releaseCandidate' is distinct from 'v105-shadow-v8-run-length-ask-road'
    ...
    raise exception 'v105-shadow-v8-run-length-ask-road issuance payload is invalid';
  end if;

  insert into public.v105_shadow_v8_issuances (
  ...
  select * into issued from public.v105_shadow_v8_issuances
  where source=p_prediction->>'source' and table_id=p_prediction->>'table_id'
    ...
    and strategy_version='v105-shadow-v8-run-length-ask-road';
  if not found or issued.prediction_payload is distinct from p_prediction->'prediction_payload' then
    raise exception 'conflicting v105-shadow-v8-run-length-ask-road issuance';
  end if;
...
create or replace function public.settle_v105_shadow_v8_prediction(p_settlement jsonb)
returns jsonb
language plpgsql
security definer
set search_path=pg_catalog,public,extensions
...
begin
  if p_settlement->>'strategy_version' is distinct from 'v105-shadow-v8-run-length-ask-road'
    ...
    raise exception 'v105-shadow-v8-run-length-ask-road settlement payload is invalid';
  end if;
  select * into issued from public.v105_shadow_v8_issuances
    where id=(p_settlement->>'prediction_id')::uuid for update;
  if not found
     or issued.strategy_version <> 'v105-shadow-v8-run-length-ask-road'
  ...
  if found then
    if existing.actual_result is distinct from p_settlement->>'actual_result'
      ...
      raise exception 'conflicting v105-shadow-v8-run-length-ask-road settlement';
    end if;
  ...
  select * into counters from public.v105_shadow_v8_sequence_counters
    where release_candidate='v105-shadow-v8-run-length-ask-road' for update;
  if not found then raise exception 'v105-shadow-v8-run-length-ask-road sequence counter is unavailable'; end if;
  next_sequence := counters.settlement_count + 1;
  insert into public.v105_shadow_v8_settlements (
  ...
  update public.v105_shadow_v8_sequence_counters
    set settlement_count=next_sequence,
      ...
      updated_at=now()
    where release_candidate='v105-shadow-v8-run-length-ask-road'
...
create or replace view public.v105_shadow_v8_history
...
left join public.v105_shadow_v8_settlements s on s.prediction_id=i.id;

revoke all on function public.issue_v105_shadow_v8_prediction(jsonb) from public,anon,authenticated,service_role;
revoke all on function public.settle_v105_shadow_v8_prediction(jsonb) from public,anon,authenticated,service_role;
grant execute on function public.issue_v105_shadow_v8_prediction(jsonb) to service_role;
grant execute on function public.settle_v105_shadow_v8_prediction(jsonb) to service_role;
revoke all on public.v105_shadow_v8_history from public,anon,authenticated,service_role;
grant select on public.v105_shadow_v8_history to service_role;
