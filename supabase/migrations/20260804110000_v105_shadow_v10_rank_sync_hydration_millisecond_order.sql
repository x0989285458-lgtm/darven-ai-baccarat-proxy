-- Align the rank-synchronized V10 compact hydration RPC with Node.js millisecond timestamp precision.
-- Additive/repeatable: replaces one read-only SECURITY DEFINER RPC and preserves every issuance/settlement.
begin;
  ...
  compact_rows as (
    ...
    cross join lateral (
      ...
      where i.table_id = t.target_table_id
        ...
        and not exists (
          select 1
          from public.v105_shadow_v10_rank_sync_settlements final_settlement
...
revoke all on function public.get_v105_shadow_v10_rank_sync_compact_history(integer) from public;
revoke all on function public.get_v105_shadow_v10_rank_sync_compact_history(integer) from anon, authenticated, service_role;
...
begin
  ...
  where function_row.oid = 'public.get_v105_shadow_v10_rank_sync_compact_history(integer)'::pg_catalog.regprocedure
  ...
  if unexpected_roles is not null then
    raise exception 'unexpected execute ACL on rank-sync V10 compact history: %', unexpected_roles
      using errcode = '42501';
  end if;
end;
$acl$;
