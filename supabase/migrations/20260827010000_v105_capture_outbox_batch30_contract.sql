...
begin
  ...
  if expected < 1 or expected > 30 or affected <> expected then
...
begin
  ...
  if pg_catalog.strpos(claim_def, 'limitgreatest(1,least(coalesce(p_limit,10),30))') = 0 then
    raise exception 'capture outbox batch30 claim contract verification failed';
  end if;
  if pg_catalog.strpos(complete_def, 'expected>30') = 0 then
    raise exception 'capture outbox batch30 completion contract verification failed';
  end if;
  if pg_catalog.strpos(fail_def, 'expected>30') = 0 then
    raise exception 'capture outbox batch30 failure contract verification failed';
  end if;
end;
$$;
