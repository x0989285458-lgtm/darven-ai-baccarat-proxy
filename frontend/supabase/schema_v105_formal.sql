-- v105 latest-only additive RPC cutover. Apply before deploying the v105-only application.
...
begin
  if (select count(*) from public.ai_strategy_versions where status = 'active') <> 1
     or not exists (select 1 from public.ai_strategy_versions where version = 'v104' and status = 'active') then
    raise exception 'v104 must be the only active predecessor before v105 migration';
  end if;
end;
$$;

CREATE OR REPLACE FUNCTION public.apply_v105_rank_ledger_event(p_event jsonb, p_ledger jsonb DEFAULT NULL::jsonb) RETURNS jsonb
...
begin
  ...
  perform pg_catalog.pg_advisory_xact_lock(
    pg_catalog.hashtextextended('v105-rank|' || v_source || '|' || v_table || '|' || v_shoe, 0)
  );
...
revoke all on function public.apply_v105_rank_ledger_event(jsonb, jsonb) from public, anon, authenticated;
grant execute on function public.apply_v105_rank_ledger_event(jsonb, jsonb) to service_role;

create or replace function public.persist_v105_settled_round(
  p_roadmap jsonb,
  p_prediction jsonb
)
returns jsonb
...
create or replace function public.get_v105_prediction_lifecycle_stats()
returns jsonb
language sql
stable
security definer
...
revoke all on function public.persist_v105_settled_round(jsonb, jsonb) from public, anon, authenticated;
grant execute on function public.persist_v105_settled_round(jsonb, jsonb) to service_role;
revoke all on function public.issue_v105_prediction(jsonb) from public, anon, authenticated;
...
revoke all on function public.reconcile_v105_prediction_lifecycle(text, text, text, integer) from public, anon, authenticated;
grant execute on function public.reconcile_v105_prediction_lifecycle(text, text, text, integer) to service_role;
revoke all on function public.get_v105_prediction_lifecycle_stats() from public, anon, authenticated;
grant execute on function public.get_v105_prediction_lifecycle_stats() to service_role;

-- Restore v104 RPC privileges explicitly for DB-first cutover, including re-runs after an older v105 finalize.
...
-- Apply finalize_v105_cutover.sql only after the v105 Proxy and Worker pass E2E.


create table if not exists public.v105_formal_release_previous_active (
  version text primary key,
  captured_at timestamptz not null default now()
);
alter table public.v105_formal_release_previous_active enable row level security;
revoke all on table public.v105_formal_release_previous_active from public, anon, authenticated;

insert into public.v105_formal_release_previous_active(version)
select 'v104'
where exists (select 1 from public.ai_strategy_versions where version = 'v104')
  and not exists (select 1 from public.v105_formal_release_previous_active)
on conflict (version) do nothing;

do $$
begin
  if (select count(*) from public.v105_formal_release_previous_active) <> 1
     or not exists (select 1 from public.v105_formal_release_previous_active where version = 'v104') then
    raise exception 'v105 predecessor provenance must contain exactly v104';
  end if;
  if not exists (select 1 from public.ai_strategy_versions where version = 'v104') then
    raise exception 'v104 strategy source is required';
  end if;
end;
$$;

update public.ai_strategy_versions set status = 'archived' where status = 'active' and version <> 'v105';
...
select
  'v105', 'active', learned_from_date, 0, total_hit_rate,
  ...
  coalesce(metrics, '{}'::jsonb) || jsonb_build_object(
    ...
    'auto_adjust', false,
    'main_strategy', 'v105_五路通用週期正式版',
    'side_strategy', 'v105_副預測沿用v104正式規則',
    ...
    'description', 'v105正式策略；主預測採方向／信心分離、當靴收縮與第五次同邊防鎖Gate，副預測完整沿用v104正式規則。'
  ),
  'Only active runtime strategy and history source for formal release v105.',
  now(), now()
from public.ai_strategy_versions where version = 'v104'
...
begin
  if (select count(*) from public.ai_strategy_versions where status = 'active') <> 1
    ...
    raise exception 'v105 must be the only active strategy';
  end if;
end;
$$;
