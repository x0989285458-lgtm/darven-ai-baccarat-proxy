-- v104 latest-only additive RPC cutover. Apply before deploying the v104-only application.
...
begin
  if (select count(*) from public.ai_strategy_versions where status = 'active') <> 1
     or not exists (select 1 from public.ai_strategy_versions where version = 'v102' and status = 'active') then
    raise exception 'v102 must be the only active predecessor before v104 migration';
  end if;
end;
$$;

CREATE OR REPLACE FUNCTION public.apply_v104_rank_ledger_event(p_event jsonb, p_ledger jsonb DEFAULT NULL::jsonb) RETURNS jsonb
...
begin
  ...
  perform pg_catalog.pg_advisory_xact_lock(
    pg_catalog.hashtextextended('v104-rank|' || v_source || '|' || v_table || '|' || v_shoe, 0)
  );
...
revoke all on function public.apply_v104_rank_ledger_event(jsonb, jsonb) from public, anon, authenticated;
grant execute on function public.apply_v104_rank_ledger_event(jsonb, jsonb) to service_role;

create or replace function public.persist_v104_settled_round(
  p_roadmap jsonb,
  p_prediction jsonb
)
returns jsonb
...
create or replace function public.issue_v104_prediction(p_prediction jsonb)
returns jsonb
language plpgsql
security definer
set search_path = public
...
create or replace function public.settle_v104_prediction(p_roadmap jsonb, p_settlement jsonb)
returns jsonb
language plpgsql
security definer
set search_path = public
...
begin
  ...
  if existing.id is null
     or existing.strategy_version is distinct from 'v104'
...
create or replace function public.reconcile_v104_prediction_lifecycle(
  p_source text,
  p_table_id text,
  p_current_shoe text,
...
create or replace function public.get_v104_prediction_lifecycle_stats()
returns jsonb
language sql
stable
security definer
...
revoke all on function public.persist_v104_settled_round(jsonb, jsonb) from public, anon, authenticated;
grant execute on function public.persist_v104_settled_round(jsonb, jsonb) to service_role;
revoke all on function public.issue_v104_prediction(jsonb) from public, anon, authenticated;
grant execute on function public.issue_v104_prediction(jsonb) to service_role;
revoke all on function public.settle_v104_prediction(jsonb, jsonb) from public, anon, authenticated;
grant execute on function public.settle_v104_prediction(jsonb, jsonb) to service_role;
revoke all on function public.reconcile_v104_prediction_lifecycle(text, text, text, integer) from public, anon, authenticated;
grant execute on function public.reconcile_v104_prediction_lifecycle(text, text, text, integer) to service_role;
revoke all on function public.get_v104_prediction_lifecycle_stats() from public, anon, authenticated;
grant execute on function public.get_v104_prediction_lifecycle_stats() to service_role;

-- Restore v102 RPC privileges explicitly for DB-first cutover, including re-runs after an older v104 finalize.
...
-- Apply finalize_v104_cutover.sql only after the v104 Proxy and Worker pass E2E.


create table if not exists public.v104_formal_release_previous_active (
  version text primary key,
  captured_at timestamptz not null default now()
);
alter table public.v104_formal_release_previous_active enable row level security;
revoke all on table public.v104_formal_release_previous_active from public, anon, authenticated;

insert into public.v104_formal_release_previous_active(version)
select 'v102'
where exists (select 1 from public.ai_strategy_versions where version = 'v102')
  and not exists (select 1 from public.v104_formal_release_previous_active)
on conflict (version) do nothing;

do $$
begin
  if (select count(*) from public.v104_formal_release_previous_active) <> 1
     or not exists (select 1 from public.v104_formal_release_previous_active where version = 'v102') then
    raise exception 'v104 predecessor provenance must contain exactly v102';
  end if;
  if not exists (select 1 from public.ai_strategy_versions where version = 'v102') then
    raise exception 'v102 strategy source is required';
  end if;
end;
$$;

update public.ai_strategy_versions set status = 'archived' where status = 'active' and version <> 'v104';
...
select
  'v104', 'active', learned_from_date, 0, total_hit_rate,
  ...
  coalesce(metrics, '{}'::jsonb) || jsonb_build_object(
    ...
    'auto_adjust', false,
    'main_strategy', 'v104_主預測防鎖邊正式版',
    'side_strategy', 'v104_副預測沿用v102正式規則',
    ...
    'side_thresholds', jsonb_build_object(
      'tie', 30,
      'superSix', 50,
      'bankerPair', 50,
      'playerPair', 50,
    ...
    'shoe_bias', jsonb_build_object('priorSampleSize', 8, 'priorCenter', 0.5, 'maximumEdge', 0.08),
    'description', 'v104正式策略；主預測採方向／信心分離、當靴收縮與第五次同邊防鎖Gate，副預測完整沿用v102正式規則。'
  ),
  'Only active runtime strategy and history source for formal release v104.',
  now(), now()
from public.ai_strategy_versions where version = 'v102'
...
begin
  if (select count(*) from public.ai_strategy_versions where status = 'active') <> 1
    ...
    raise exception 'v104 must be the only active strategy';
  end if;
end;
$$;
