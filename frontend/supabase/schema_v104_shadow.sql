-- v104.0.0-shadow.1 additive shadow schema. Apply after v103 shadow; v102 remains Active.
begin;
...
begin
  if (select count(*) from public.ai_strategy_versions where status = 'active') <> 1
    ...
    raise exception 'v102 must remain the only Active strategy for v104 shadow';
  end if;
end;
$$;

create table if not exists public.v104_shadow_runtime_settings (
  release_candidate text primary key,
  strategy_version text not null check (strategy_version = 'v104'),
...
create table if not exists public.v104_shadow_issuances (
  ...
  constraint v104_shadow_issuance_identity_unique unique (source, table_id, shoe_no, round_no, strategy_version)
);

create table if not exists public.v104_shadow_settlements (
  id uuid primary key default gen_random_uuid(),
  prediction_id uuid not null unique references public.v104_shadow_issuances(id),
  source text not null,
  table_id text not null,
  shoe_no text not null,
  round_no integer not null check (round_no > 0),
  ...
  created_at timestamptz not null default now(),
  constraint v104_shadow_push_semantics check (
...
alter table public.v104_shadow_runtime_settings enable row level security;
alter table public.v104_shadow_issuances enable row level security;
alter table public.v104_shadow_settlements enable row level security;

revoke all on table public.v104_shadow_runtime_settings from public, anon, authenticated;
revoke all on table public.v104_shadow_issuances from public, anon, authenticated;
revoke all on table public.v104_shadow_settlements from public, anon, authenticated;
revoke all on table public.v104_shadow_runtime_settings from service_role;
revoke all on table public.v104_shadow_issuances from service_role;
revoke all on table public.v104_shadow_settlements from service_role;
grant select on table public.v104_shadow_runtime_settings to service_role;
grant select on table public.v104_shadow_issuances to service_role;
grant select on table public.v104_shadow_settlements to service_role;

insert into public.v104_shadow_runtime_settings (
  release_candidate, strategy_version, status, enabled, active_strategy_version, updated_at
) values ('v104.0.0-shadow.1', 'v104', 'shadow', true, 'v102', now())
on conflict (release_candidate) do update set
  strategy_version = excluded.strategy_version, status = 'shadow', enabled = true,
  active_strategy_version = 'v102', updated_at = now();

create or replace function public.issue_v104_shadow_prediction(p_prediction jsonb)
returns jsonb
language plpgsql
security definer
...
as $$
declare
  issued public.v104_shadow_issuances%rowtype;
  inserted boolean := false;
begin
  if not exists (
    select 1 from public.v104_shadow_runtime_settings
    where release_candidate = 'v104.0.0-shadow.1' and enabled = true and status = 'shadow' and active_strategy_version = 'v102'
  ) then raise exception 'v104 shadow is disabled'; end if;
  ...
  if nullif(p_prediction->>'source', '') is null
     ...
     or p_prediction->>'strategy_version' is distinct from 'v104'
     ...
     or p_prediction->'prediction_payload'->>'strategyVersion' is distinct from 'v104'
     ...
     or p_prediction->'prediction_payload'->'diagnostics'->'calibration'->>'directionContribution' is distinct from '0'
    ...
    raise exception 'v104 shadow issuance payload is incomplete';
  end if;

  insert into public.v104_shadow_issuances (
    source, table_id, shoe_no, round_no, strategy_version, prediction_timing,
    predicted_result, confidence, feature_weights, score_sources, score_totals, diagnostics,
    ...
    'v104', 'pre_result_context', p_prediction->>'predicted_result', (p_prediction->>'confidence')::integer,
    p_prediction->'feature_weights', p_prediction->'score_sources', p_prediction->'score_totals', p_prediction->'diagnostics',
  ...
  if not inserted then
    select * into issued from public.v104_shadow_issuances
    where source = p_prediction->>'source' and table_id = p_prediction->>'table_id'
      ...
      and strategy_version = 'v104';
    if issued.id is null or issued.prediction_timing is distinct from 'pre_result_context'
       ...
       or issued.diagnostics is distinct from p_prediction->'diagnostics'
       or issued.same_side_streak is distinct from (p_prediction->>'same_side_streak')::integer
       or issued.independent_support_count is distinct from (p_prediction->>'independent_support_count')::integer
       or issued.shoe_bias_suppressed is distinct from (p_prediction->>'shoe_bias_suppressed')::boolean
       or issued.lock_risk is distinct from (p_prediction->>'lock_risk')::boolean
       or issued.prediction_payload is distinct from p_prediction->'prediction_payload' then
      raise exception 'conflicting v104 shadow issuance';
    end if;
  end if;
...
create or replace function public.settle_v104_shadow_prediction(p_settlement jsonb)
returns jsonb
language plpgsql
security definer
set search_path = pg_catalog, public, extensions
...
declare
  issued public.v104_shadow_issuances%rowtype;
  settled public.v104_shadow_settlements%rowtype;
  inserted boolean := false;
...
begin
  ...
  ) then raise exception 'v104 shadow settlement requires verified Final summary/show_win; show_poker is provisional'; end if;
  ...
  if issued.id is null or issued.source is distinct from p_settlement->>'source'
    ...
    raise exception 'v104 shadow settlement identity mismatch';
  end if;
  ...
  if p_settlement->>'settlement_status' is distinct from expected_status
    ...
    raise exception 'v104 shadow settlement outcome is inconsistent';
  end if;

  insert into public.v104_shadow_settlements (
  ...
  if not inserted then
    select * into settled from public.v104_shadow_settlements where prediction_id = issued.id;
    if settled.actual_result is distinct from p_settlement->>'actual_result'
      ...
      raise exception 'conflicting v104 shadow settlement';
    end if;
  end if;
...
create or replace view public.v104_shadow_history as
select
  i.id as prediction_id, i.source, i.table_id, i.shoe_no, i.round_no,
  i.strategy_version, i.prediction_timing, i.prediction_issued_at, i.predicted_result, i.confidence,
  i.prediction_payload,
  i.same_side_streak, i.independent_support_count, i.shoe_bias_suppressed, i.lock_risk,
  s.actual_result, s.is_hit, s.settlement_status, coalesce(s.settlement_final, false) as settlement_final,
  s.settlement_source_action, s.resolved_at
from public.v104_shadow_issuances i
left join public.v104_shadow_settlements s on s.prediction_id = i.id
where i.strategy_version = 'v104' and i.prediction_timing = 'pre_result_context'
  and i.prediction_issued_at is not null;

revoke all on table public.v104_shadow_history from public, anon, authenticated;
revoke all on table public.v104_shadow_history from service_role;
grant select on table public.v104_shadow_history to service_role;

create or replace function public.get_v104_shadow_history(p_limit integer default 10000)
returns setof public.v104_shadow_history
language sql
stable
security definer
set search_path = pg_catalog, public, extensions
as $$
  select recent.*
  from (
    select * from public.v104_shadow_history
    order by prediction_issued_at desc
    limit least(10000, greatest(1, coalesce(p_limit, 10000)))
  ) recent
  order by prediction_issued_at asc;
$$;

revoke all on function public.issue_v104_shadow_prediction(jsonb) from public, anon, authenticated;
revoke all on function public.settle_v104_shadow_prediction(jsonb) from public, anon, authenticated;
revoke all on function public.get_v104_shadow_history(integer) from public, anon, authenticated;
grant execute on function public.issue_v104_shadow_prediction(jsonb) to service_role;
grant execute on function public.settle_v104_shadow_prediction(jsonb) to service_role;
grant execute on function public.get_v104_shadow_history(integer) to service_role;
