-- v104.1.0 seven-head iteration shadow; additive and isolated from formal v104.
begin;
...
create table if not exists public.v104_iteration_shadow_runtime_settings (
  release_candidate text primary key,
  strategy_version text not null check (strategy_version = 'v104-seven-head-shadow-v1'),
...
create table if not exists public.v104_iteration_shadow_sequence_counters (
  release_candidate text primary key,
...
create table if not exists public.v104_iteration_shadow_issuances (
  ...
  constraint v104_iteration_shadow_issuance_identity_unique unique (source, table_id, shoe_no, round_no, strategy_version)
);

create table if not exists public.v104_iteration_shadow_settlements (
  id uuid primary key default gen_random_uuid(),
  settlement_sequence bigint not null unique,
  prediction_id uuid not null unique references public.v104_iteration_shadow_issuances(id),
  source text not null,
  table_id text not null,
  shoe_no text not null,
  round_no integer not null check (round_no > 0),
  ...
  created_at timestamptz not null default now(),
  constraint v104_iteration_shadow_main_semantics check (
...
create table if not exists public.v104_iteration_shadow_cycle_reports (
  ...
  model_version text not null check (model_version = 'v104-seven-head-shadow-v1'),
  report_payload jsonb not null,
  report_svg text not null,
  created_at timestamptz not null default now(),
  constraint v104_iteration_shadow_cycle_exact check (
    start_sequence = ((cycle_number - 1) * 1000 + 1)
    and end_sequence = cycle_number * 1000
  )
);

create table if not exists public.v104_iteration_shadow_weight_suggestions (
  ...
  constraint v104_iteration_shadow_action_cycle_exact check (
    sample_start_action = ((action_cycle - 1) * 1000 + 1)
    and sample_end_action = action_cycle * 1000
  )
);

alter table public.v104_iteration_shadow_runtime_settings enable row level security;
alter table public.v104_iteration_shadow_sequence_counters enable row level security;
alter table public.v104_iteration_shadow_issuances enable row level security;
alter table public.v104_iteration_shadow_settlements enable row level security;
alter table public.v104_iteration_shadow_cycle_reports enable row level security;
alter table public.v104_iteration_shadow_weight_suggestions enable row level security;

revoke all on table public.v104_iteration_shadow_runtime_settings from public, anon, authenticated, service_role;
revoke all on table public.v104_iteration_shadow_sequence_counters from public, anon, authenticated, service_role;
revoke all on table public.v104_iteration_shadow_issuances from public, anon, authenticated, service_role;
revoke all on table public.v104_iteration_shadow_settlements from public, anon, authenticated, service_role;
revoke all on table public.v104_iteration_shadow_cycle_reports from public, anon, authenticated, service_role;
revoke all on table public.v104_iteration_shadow_weight_suggestions from public, anon, authenticated, service_role;
grant select on table public.v104_iteration_shadow_runtime_settings to service_role;
grant select on table public.v104_iteration_shadow_sequence_counters to service_role;
grant select on table public.v104_iteration_shadow_issuances to service_role;
grant select on table public.v104_iteration_shadow_settlements to service_role;
grant select on table public.v104_iteration_shadow_cycle_reports to service_role;
grant select on table public.v104_iteration_shadow_weight_suggestions to service_role;

insert into public.v104_iteration_shadow_runtime_settings (
  release_candidate, strategy_version, status, enabled, active_strategy_version, updated_at
) values ('v104.1.0-seven-head-shadow.1', 'v104-seven-head-shadow-v1', 'shadow', true, 'v104', now())
...
insert into public.v104_iteration_shadow_sequence_counters (release_candidate)
values ('v104.1.0-seven-head-shadow.1') on conflict (release_candidate) do nothing;

create or replace function public.issue_v104_iteration_shadow_prediction(p_prediction jsonb)
returns jsonb language plpgsql security definer set search_path=pg_catalog,public,extensions as $$
declare issued public.v104_iteration_shadow_issuances%rowtype; inserted boolean := false;
begin
  if (select count(*) from public.ai_strategy_versions where status='active') <> 1
     or not exists (select 1 from public.ai_strategy_versions where status='active' and version='v104') then
    raise exception 'v104 Active strategy verification failed';
  end if;
  if not exists (select 1 from public.v104_iteration_shadow_runtime_settings
    where release_candidate='v104.1.0-seven-head-shadow.1' and enabled=true and status='shadow' and active_strategy_version='v104') then
  ...
  if nullif(p_prediction->>'source','') is null or nullif(p_prediction->>'table_id','') is null
     ...
     or p_prediction->>'strategy_version' is distinct from 'v104-seven-head-shadow-v1'
     ...
     or p_prediction->'prediction_payload'->>'strategyVersion' is distinct from 'v104-seven-head-shadow-v1'
  ...
  insert into public.v104_iteration_shadow_issuances (
    ...
    'v104-seven-head-shadow-v1','pre_result_context',p_prediction->>'predicted_result',(p_prediction->>'confidence')::numeric,
  ...
  if not inserted then
    select * into issued from public.v104_iteration_shadow_issuances where source=p_prediction->>'source'
      and table_id=p_prediction->>'table_id' and shoe_no=p_prediction->>'shoe_no'
      and round_no=(p_prediction->>'round_no')::integer and strategy_version='v104-seven-head-shadow-v1';
...
create or replace function public.settle_v104_iteration_shadow_prediction(p_settlement jsonb)
returns jsonb language plpgsql security definer set search_path=pg_catalog,public,extensions as $$
declare
  issued public.v104_iteration_shadow_issuances%rowtype;
  settled public.v104_iteration_shadow_settlements%rowtype;
  counter public.v104_iteration_shadow_sequence_counters%rowtype;
...
begin
  ...
  select * into issued from public.v104_iteration_shadow_issuances where id=(p_settlement->>'prediction_id')::uuid for update;
  ...
  select * into settled from public.v104_iteration_shadow_settlements where prediction_id=issued.id;
  if settled.id is not null then
    ...
    select * into counter from public.v104_iteration_shadow_sequence_counters where release_candidate='v104.1.0-seven-head-shadow.1';
  ...
  select * into counter from public.v104_iteration_shadow_sequence_counters
    where release_candidate='v104.1.0-seven-head-shadow.1' for update;
  update public.v104_iteration_shadow_sequence_counters set
    ...
    updated_at=now()
    where release_candidate='v104.1.0-seven-head-shadow.1' returning * into counter;
  insert into public.v104_iteration_shadow_settlements (
...
create or replace function public.persist_v104_iteration_shadow_artifacts(p_report jsonb, p_suggestions jsonb)
returns jsonb language plpgsql security definer set search_path=pg_catalog,public,extensions as $$
declare
  report_payload jsonb; report_svg text; cycle_no bigint; existing_report public.v104_iteration_shadow_cycle_reports%rowtype;
  suggestion jsonb; existing_suggestion public.v104_iteration_shadow_weight_suggestions%rowtype;
...
begin
  ...
  if p_report is not null and p_report <> 'null'::jsonb then
    ...
    if jsonb_typeof(report_payload) is distinct from 'object' or coalesce(report_svg,'') not like '<svg%'
       or (select count(*) from public.v104_iteration_shadow_settlements where settlement_sequence between ((cycle_no-1)*1000+1) and cycle_no*1000) <> 1000 then
      raise exception 'v104 iteration shadow cycle report is incomplete';
    end if;
    insert into public.v104_iteration_shadow_cycle_reports (
      cycle_number,start_sequence,end_sequence,model_version,report_payload,report_svg
    ) values (cycle_no,(cycle_no-1)*1000+1,cycle_no*1000,'v104-seven-head-shadow-v1',report_payload,report_svg)
    on conflict (cycle_number) do nothing returning * into existing_report;
    if existing_report.id is null then
      select * into existing_report from public.v104_iteration_shadow_cycle_reports where cycle_number=cycle_no;
  ...
  if p_suggestions is not null and jsonb_typeof(p_suggestions)='array' then
    for suggestion in select value from jsonb_array_elements(p_suggestions) loop
      ...
      if head_key_value not in ('main','tie','superSix','bankerDragon','playerDragon','bankerPair','playerPair')
         or suggestion->>'modelVersion' is distinct from 'v104-seven-head-shadow-v1'
         ...
         or (select count(*) from public.v104_iteration_shadow_settlements where (head_results->head_key_value->>'action')::boolean) < sample_end
      ...
      insert into public.v104_iteration_shadow_weight_suggestions (
        ...
        suggestion->>'id',head_key_value,action_cycle_value,sample_start,sample_end,'v104-seven-head-shadow-v1','exhaustive_5_percent_grid',
      ...
      if existing_suggestion.suggestion_id is null then
        select * into existing_suggestion from public.v104_iteration_shadow_weight_suggestions
...
create or replace function public.review_v104_iteration_shadow_suggestion(p_suggestion_id text, p_decision text, p_reviewer text)
returns jsonb language plpgsql security definer set search_path=pg_catalog,public,extensions as $$
declare reviewed public.v104_iteration_shadow_weight_suggestions%rowtype;
begin
  if not exists (select 1 from public.v104_iteration_shadow_runtime_settings
    where release_candidate='v104.1.0-seven-head-shadow.1' and enabled=true and status='shadow' and active_strategy_version='v104') then
    raise exception 'v104 iteration shadow is disabled';
  ...
  update public.v104_iteration_shadow_weight_suggestions set status=p_decision,reviewed_by=p_reviewer,reviewed_at=now()
...
create or replace view public.v104_iteration_shadow_history as
...
from public.v104_iteration_shadow_issuances i left join public.v104_iteration_shadow_settlements s on s.prediction_id=i.id
where i.strategy_version='v104-seven-head-shadow-v1' and i.prediction_timing='pre_result_context' and i.prediction_issued_at is not null;

revoke all on table public.v104_iteration_shadow_history from public,anon,authenticated,service_role;
grant select on table public.v104_iteration_shadow_history to service_role;

revoke all on function public.issue_v104_iteration_shadow_prediction(jsonb) from public,anon,authenticated;
revoke all on function public.settle_v104_iteration_shadow_prediction(jsonb) from public,anon,authenticated;
revoke all on function public.persist_v104_iteration_shadow_artifacts(jsonb,jsonb) from public,anon,authenticated;
revoke all on function public.review_v104_iteration_shadow_suggestion(text,text,text) from public,anon,authenticated;
grant execute on function public.issue_v104_iteration_shadow_prediction(jsonb) to service_role;
grant execute on function public.settle_v104_iteration_shadow_prediction(jsonb) to service_role;
grant execute on function public.persist_v104_iteration_shadow_artifacts(jsonb,jsonb) to service_role;
grant execute on function public.review_v104_iteration_shadow_suggestion(text,text,text) to service_role;
