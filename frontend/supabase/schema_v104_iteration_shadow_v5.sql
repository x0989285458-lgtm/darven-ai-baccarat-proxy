-- v104.5.0 seven-head iteration shadow v5; additive and isolated from formal v104 and v1-v4 shadow evidence.
begin;
...
begin
  if to_regclass('public.v104_iteration_shadow_v4_runtime_settings') is null then
    raise exception 'v4 must be fully stopped before v5 activation';
  end if;
...
create table if not exists public.v104_iteration_shadow_v5_runtime_settings (
  release_candidate text primary key,
  strategy_version text not null check (strategy_version = 'v104-seven-head-shadow-v5-best-stage-side-reweight'),
...
alter table public.v104_iteration_shadow_v5_runtime_settings
  drop constraint if exists v104_iteration_shadow_v5_runtime_settings_status_check;
alter table public.v104_iteration_shadow_v5_runtime_settings
  add constraint v104_iteration_shadow_v5_runtime_settings_status_check
  check (status in ('shadow', 'draining', 'shadow_disabled'));

create table if not exists public.v104_iteration_shadow_v5_sequence_counters (
  release_candidate text primary key,
...
create table if not exists public.v104_iteration_shadow_v5_issuances (
  ...
  constraint v104_iteration_shadow_v5_issuance_identity_unique unique (source, table_id, shoe_no, round_no, strategy_version)
);

create table if not exists public.v104_iteration_shadow_v5_settlements (
  id uuid primary key default gen_random_uuid(),
  settlement_sequence bigint not null unique,
  prediction_id uuid not null unique references public.v104_iteration_shadow_v5_issuances(id),
  source text not null,
  table_id text not null,
  shoe_no text not null,
  round_no integer not null check (round_no > 0),
  ...
  created_at timestamptz not null default now(),
  constraint v104_iteration_shadow_v5_main_semantics check (
...
create table if not exists public.v104_iteration_shadow_v5_cycle_reports (
  id uuid primary key default gen_random_uuid(),
  cycle_number bigint not null unique check (cycle_number > 0),
  start_sequence bigint not null,
  end_sequence bigint not null,
  model_version text not null check (model_version = 'v104-seven-head-shadow-v5-best-stage-side-reweight'),
  report_payload jsonb not null,
  report_svg text not null,
  created_at timestamptz not null default now(),
  constraint v104_iteration_shadow_v5_cycle_exact check (
    start_sequence = ((cycle_number - 1) * 1000 + 1)
    and end_sequence = cycle_number * 1000
  )
);

create table if not exists public.v104_iteration_shadow_v5_weight_suggestions (
  ...
  constraint v104_iteration_shadow_v5_action_cycle_exact check (
    sample_start_action = ((action_cycle - 1) * 1000 + 1)
    and sample_end_action = action_cycle * 1000
  )
);

alter table public.v104_iteration_shadow_v5_runtime_settings enable row level security;
alter table public.v104_iteration_shadow_v5_sequence_counters enable row level security;
alter table public.v104_iteration_shadow_v5_issuances enable row level security;
alter table public.v104_iteration_shadow_v5_settlements enable row level security;
alter table public.v104_iteration_shadow_v5_cycle_reports enable row level security;
alter table public.v104_iteration_shadow_v5_weight_suggestions enable row level security;

revoke all on table public.v104_iteration_shadow_v5_runtime_settings from public, anon, authenticated, service_role;
revoke all on table public.v104_iteration_shadow_v5_sequence_counters from public, anon, authenticated, service_role;
revoke all on table public.v104_iteration_shadow_v5_issuances from public, anon, authenticated, service_role;
revoke all on table public.v104_iteration_shadow_v5_settlements from public, anon, authenticated, service_role;
revoke all on table public.v104_iteration_shadow_v5_cycle_reports from public, anon, authenticated, service_role;
revoke all on table public.v104_iteration_shadow_v5_weight_suggestions from public, anon, authenticated, service_role;
grant select on table public.v104_iteration_shadow_v5_runtime_settings to service_role;
grant select on table public.v104_iteration_shadow_v5_sequence_counters to service_role;
grant select on table public.v104_iteration_shadow_v5_issuances to service_role;
grant select on table public.v104_iteration_shadow_v5_settlements to service_role;
grant select on table public.v104_iteration_shadow_v5_cycle_reports to service_role;
grant select on table public.v104_iteration_shadow_v5_weight_suggestions to service_role;

insert into public.v104_iteration_shadow_v5_runtime_settings (
  release_candidate, strategy_version, status, enabled, active_strategy_version, updated_at
) values ('v104.5.0-seven-head-shadow.5', 'v104-seven-head-shadow-v5-best-stage-side-reweight', 'shadow', true, 'v104', now())
on conflict (release_candidate) do nothing;

insert into public.v104_iteration_shadow_v5_sequence_counters (release_candidate)
values ('v104.5.0-seven-head-shadow.5') on conflict (release_candidate) do nothing;

create or replace function public.issue_v104_iteration_shadow_v5_prediction(p_prediction jsonb)
returns jsonb language plpgsql security definer set search_path=pg_catalog,public,extensions as $$
declare
  issued public.v104_iteration_shadow_v5_issuances%rowtype;
...
begin
  ...
  perform 1 from public.v104_iteration_shadow_v5_runtime_settings
    where release_candidate='v104.5.0-seven-head-shadow.5' and enabled=true and status='shadow' and active_strategy_version='v104'
    for share;
  if not found then
  ...
  if nullif(p_prediction->>'source','') is null or nullif(p_prediction->>'table_id','') is null
     ...
     or p_prediction->>'strategy_version' is distinct from 'v104-seven-head-shadow-v5-best-stage-side-reweight'
     ...
     or p_prediction->'prediction_payload'->>'strategyVersion' is distinct from 'v104-seven-head-shadow-v5-best-stage-side-reweight'
     or p_prediction->'prediction_payload'->>'releaseCandidate' is distinct from 'v104.5.0-seven-head-shadow.5'
     ...
     or p_prediction->'prediction_payload'->'heads'->'main'->>'sourceVersion' is distinct from 'v4-unchanged'
     or p_prediction->'prediction_payload'->'heads'->'tie'->>'sourceVersion' is distinct from 'v4-unchanged'
     or p_prediction->'prediction_payload'->'heads'->'superSix'->>'sourceVersion' is distinct from 'v4-stage-9-reweight'
     or p_prediction->'prediction_payload'->'heads'->'bankerDragon'->>'sourceVersion' is distinct from 'v4-stage-9-reweight'
     or p_prediction->'prediction_payload'->'heads'->'playerDragon'->>'sourceVersion' is distinct from 'v4-unchanged'
     or p_prediction->'prediction_payload'->'heads'->'bankerPair'->>'sourceVersion' is distinct from 'v4-unchanged'
     or p_prediction->'prediction_payload'->'heads'->'playerPair'->>'sourceVersion' is distinct from 'v4-stage-6-reweight'
     ...
     or p_prediction->'prediction_payload'->'heads'->'main'->'weights' is distinct from '{"roadmap_trend_signals":0.275,"ask_road_signals":0.275,"shoe_banker_player_bias":0.35,"neutral_reserve":0.10}'::jsonb
     or p_prediction->'prediction_payload'->'heads'->'tie'->'weights' is distinct from '{"tie_count":0.10,"banker_pair_count":0,"player_pair_count":0,"bead_road":0,"big_road":0,"big_eye_road":0,"small_road":0,"cockroach_road":0,"next_banker_road":0,"next_player_road":0,"shoe":0,"round":0,"shoe_stage":0.10,"player_point":0,"banker_point":0,"point_diff":0,"banker_natural":0,"player_natural":0,"banker_dragon":0,"player_dragon":0,"super_six":0,"tie_risk":0.45,"pair_risk":0,"ask_road_conflict":0,"road_chaos":0.15,"table_side_history":0,"remaining_rank_pressure":0,"remaining_rank_total":0.20}'::jsonb
     or p_prediction->'prediction_payload'->'heads'->'superSix'->'weights' is distinct from '{"tie_count":0,"banker_pair_count":0,"player_pair_count":0,"bead_road":0,"big_road":0,"big_eye_road":0,"small_road":0,"cockroach_road":0,"next_banker_road":0,"next_player_road":0,"shoe":0,"round":0,"shoe_stage":0.10,"player_point":0,"banker_point":0.30,"point_diff":0,"banker_natural":0,"player_natural":0,"banker_dragon":0,"player_dragon":0,"super_six":0,"tie_risk":0,"pair_risk":0,"ask_road_conflict":0,"road_chaos":0,"table_side_history":0.25,"remaining_rank_pressure":0,"remaining_rank_total":0.35}'::jsonb
     or p_prediction->'prediction_payload'->'heads'->'bankerDragon'->'weights' is distinct from '{"tie_count":0,"banker_pair_count":0,"player_pair_count":0,"bead_road":0,"big_road":0.15,"big_eye_road":0,"small_road":0,"cockroach_road":0,"next_banker_road":0,"next_player_road":0,"shoe":0,"round":0,"shoe_stage":0,"player_point":0,"banker_point":0.35,"point_diff":0.10,"banker_natural":0.05,"player_natural":0,"banker_dragon":0,"player_dragon":0,"super_six":0,"tie_risk":0,"pair_risk":0,"ask_road_conflict":0,"road_chaos":0,"table_side_history":0,"remaining_rank_pressure":0,"remaining_rank_total":0.35}'::jsonb
     or p_prediction->'prediction_payload'->'heads'->'playerDragon'->'weights' is distinct from '{"tie_count":0,"banker_pair_count":0,"player_pair_count":0,"bead_road":0,"big_road":0.10,"big_eye_road":0,"small_road":0,"cockroach_road":0,"next_banker_road":0,"next_player_road":0,"shoe":0,"round":0,"shoe_stage":0,"player_point":0.35,"banker_point":0,"point_diff":0.15,"banker_natural":0,"player_natural":0.10,"banker_dragon":0,"player_dragon":0,"super_six":0,"tie_risk":0,"pair_risk":0,"ask_road_conflict":0,"road_chaos":0,"table_side_history":0,"remaining_rank_pressure":0,"remaining_rank_total":0.30}'::jsonb
     or p_prediction->'prediction_payload'->'heads'->'bankerPair'->'weights' is distinct from '{"tie_count":0,"banker_pair_count":0.20,"player_pair_count":0,"bead_road":0,"big_road":0,"big_eye_road":0,"small_road":0,"cockroach_road":0,"next_banker_road":0,"next_player_road":0,"shoe":0,"round":0,"shoe_stage":0.20,"player_point":0,"banker_point":0,"point_diff":0,"banker_natural":0,"player_natural":0,"banker_dragon":0,"player_dragon":0,"super_six":0,"tie_risk":0,"pair_risk":0.35,"ask_road_conflict":0,"road_chaos":0,"table_side_history":0.10,"remaining_rank_pressure":0.15,"remaining_rank_total":0}'::jsonb
     or p_prediction->'prediction_payload'->'heads'->'playerPair'->'weights' is distinct from '{"tie_count":0,"banker_pair_count":0,"player_pair_count":0.20,"bead_road":0,"big_road":0,"big_eye_road":0,"small_road":0,"cockroach_road":0,"next_banker_road":0,"next_player_road":0,"shoe":0,"round":0,"shoe_stage":0.15,"player_point":0,"banker_point":0,"point_diff":0,"banker_natural":0,"player_natural":0,"banker_dragon":0,"player_dragon":0,"super_six":0,"tie_risk":0,"pair_risk":0.20,"ask_road_conflict":0,"road_chaos":0,"table_side_history":0.25,"remaining_rank_pressure":0.20,"remaining_rank_total":0}'::jsonb
  ...
  if p_prediction->'prediction_payload'->'heads'->'main'->'action' is distinct from 'true'::jsonb
     or p_prediction->'prediction_payload'->'heads'->'main'->>'predictedResult' is distinct from p_prediction->>'predicted_result'
     or (p_prediction->'prediction_payload'->'heads'->'main'->>'confidence')::numeric is distinct from (p_prediction->>'confidence')::numeric
     or (p_prediction->'prediction_payload'->'heads'->'main'->>'units')::integer is distinct from greatest(1,least(5,round(1+((greatest(50,least(70,(p_prediction->>'confidence')::numeric))-50)*4/20))::integer)) then
    raise exception 'v104 iteration shadow main head contract mismatch';
  end if;

  foreach head_key in array array['tie','superSix','bankerDragon','playerDragon','bankerPair','playerPair'] loop
    side_head := p_prediction->'prediction_payload'->'heads'->head_key;
    expected_threshold := (side_head->>'threshold')::numeric;
    if jsonb_typeof(side_head->'featureValues') is distinct from 'object'
       or (select array_agg(key order by key) from jsonb_object_keys(side_head->'featureValues') as feature_keys(key))
          is distinct from (select array_agg(key order by key) from jsonb_object_keys(side_head->'weights') as weight_keys(key))
       or exists (select 1 from jsonb_each_text(side_head->'featureValues') as features(key,value)
                  where value::numeric < 0 or value::numeric > 100) then
      raise exception 'v104 iteration shadow side feature contract mismatch';
    end if;
    select sum((weight.value #>> '{}')::numeric * (side_head->'featureValues'->>weight.key)::numeric)
      into expected_confidence from jsonb_each(side_head->'weights') as weight(key,value);
    expected_confidence := greatest(0,least(100,expected_confidence));
    expected_action := coalesce((side_head->>'rankAvailable')::boolean,false) and expected_confidence >= expected_threshold;
    expected_units := case when expected_action then
      greatest(1,least(10,round(1+((expected_confidence-expected_threshold)*9/(100-expected_threshold)))::integer))
      else 0 end;
    if coalesce(abs((side_head->>'confidence')::numeric-expected_confidence)>0.000000001,true)
       or side_head->'action' is distinct from to_jsonb(expected_action)
       or (side_head->>'units')::integer is distinct from expected_units then
      raise exception 'v104 iteration shadow side confidence/action/units mismatch';
    end if;
  end loop;

  insert into public.v104_iteration_shadow_v5_issuances (
    ...
    'v104-seven-head-shadow-v5-best-stage-side-reweight','pre_result_context',p_prediction->>'predicted_result',(p_prediction->>'confidence')::numeric,
  ...
  if not inserted then
    select * into issued from public.v104_iteration_shadow_v5_issuances where source=p_prediction->>'source'
      and table_id=p_prediction->>'table_id' and shoe_no=p_prediction->>'shoe_no'
      and round_no=(p_prediction->>'round_no')::integer and strategy_version='v104-seven-head-shadow-v5-best-stage-side-reweight';
...
create or replace function public.settle_v104_iteration_shadow_v5_prediction(p_settlement jsonb)
returns jsonb language plpgsql security definer set search_path=pg_catalog,public,extensions as $$
declare
  issued public.v104_iteration_shadow_v5_issuances%rowtype;
  settled public.v104_iteration_shadow_v5_settlements%rowtype;
  counter public.v104_iteration_shadow_v5_sequence_counters%rowtype;
...
begin
  perform 1 from public.v104_iteration_shadow_v5_runtime_settings
    where release_candidate='v104.5.0-seven-head-shadow.5' and enabled=true and status in ('shadow','draining') and active_strategy_version='v104'
    for share;
  if not found then
  ...
  select * into issued from public.v104_iteration_shadow_v5_issuances where id=(p_settlement->>'prediction_id')::uuid for update;
  ...
  select * into settled from public.v104_iteration_shadow_v5_settlements where prediction_id=issued.id;
  ...
  select * into counter from public.v104_iteration_shadow_v5_sequence_counters
    where release_candidate='v104.5.0-seven-head-shadow.5' for update;
  update public.v104_iteration_shadow_v5_sequence_counters set
    ...
    updated_at=now()
    where release_candidate='v104.5.0-seven-head-shadow.5' returning * into counter;
  insert into public.v104_iteration_shadow_v5_settlements (
...
create or replace function public.persist_v104_iteration_shadow_v5_artifacts(p_report jsonb, p_suggestions jsonb)
returns jsonb language plpgsql security definer set search_path=pg_catalog,public,extensions as $$
declare
  report_payload jsonb; report_svg text; cycle_no bigint; existing_report public.v104_iteration_shadow_v5_cycle_reports%rowtype;
  suggestion jsonb; existing_suggestion public.v104_iteration_shadow_v5_weight_suggestions%rowtype;
  expected_current_weights jsonb;
...
begin
  ...
  if p_suggestions is not null and p_suggestions <> 'null'::jsonb and jsonb_typeof(p_suggestions) is distinct from 'array' then
    raise exception 'v104 iteration shadow suggestions must be an array';
  end if;
  if p_report is not null and p_report <> 'null'::jsonb then
    ...
    insert into public.v104_iteration_shadow_v5_cycle_reports (
      cycle_number,start_sequence,end_sequence,model_version,report_payload,report_svg
    ) values (cycle_no,(cycle_no-1)*1000+1,cycle_no*1000,'v104-seven-head-shadow-v5-best-stage-side-reweight',report_payload,report_svg)
    on conflict (cycle_number) do nothing returning * into existing_report;
    if existing_report.id is null then
      select * into existing_report from public.v104_iteration_shadow_v5_cycle_reports where cycle_number=cycle_no;
  ...
  if p_suggestions is not null and jsonb_typeof(p_suggestions)='array' then
    for suggestion in select value from jsonb_array_elements(p_suggestions) loop
      ...
      expected_current_weights := case head_key_value
        when 'main' then '{"roadmap_trend_signals":0.275,"ask_road_signals":0.275,"shoe_banker_player_bias":0.35,"neutral_reserve":0.10}'::jsonb
        when 'tie' then '{"tie_count":0.10,"banker_pair_count":0,"player_pair_count":0,"bead_road":0,"big_road":0,"big_eye_road":0,"small_road":0,"cockroach_road":0,"next_banker_road":0,"next_player_road":0,"shoe":0,"round":0,"shoe_stage":0.10,"player_point":0,"banker_point":0,"point_diff":0,"banker_natural":0,"player_natural":0,"banker_dragon":0,"player_dragon":0,"super_six":0,"tie_risk":0.45,"pair_risk":0,"ask_road_conflict":0,"road_chaos":0.15,"table_side_history":0,"remaining_rank_pressure":0,"remaining_rank_total":0.20}'::jsonb
        when 'superSix' then '{"tie_count":0,"banker_pair_count":0,"player_pair_count":0,"bead_road":0,"big_road":0,"big_eye_road":0,"small_road":0,"cockroach_road":0,"next_banker_road":0,"next_player_road":0,"shoe":0,"round":0,"shoe_stage":0.10,"player_point":0,"banker_point":0.30,"point_diff":0,"banker_natural":0,"player_natural":0,"banker_dragon":0,"player_dragon":0,"super_six":0,"tie_risk":0,"pair_risk":0,"ask_road_conflict":0,"road_chaos":0,"table_side_history":0.25,"remaining_rank_pressure":0,"remaining_rank_total":0.35}'::jsonb
        when 'bankerDragon' then '{"tie_count":0,"banker_pair_count":0,"player_pair_count":0,"bead_road":0,"big_road":0.15,"big_eye_road":0,"small_road":0,"cockroach_road":0,"next_banker_road":0,"next_player_road":0,"shoe":0,"round":0,"shoe_stage":0,"player_point":0,"banker_point":0.35,"point_diff":0.10,"banker_natural":0.05,"player_natural":0,"banker_dragon":0,"player_dragon":0,"super_six":0,"tie_risk":0,"pair_risk":0,"ask_road_conflict":0,"road_chaos":0,"table_side_history":0,"remaining_rank_pressure":0,"remaining_rank_total":0.35}'::jsonb
        when 'playerDragon' then '{"tie_count":0,"banker_pair_count":0,"player_pair_count":0,"bead_road":0,"big_road":0.10,"big_eye_road":0,"small_road":0,"cockroach_road":0,"next_banker_road":0,"next_player_road":0,"shoe":0,"round":0,"shoe_stage":0,"player_point":0.35,"banker_point":0,"point_diff":0.15,"banker_natural":0,"player_natural":0.10,"banker_dragon":0,"player_dragon":0,"super_six":0,"tie_risk":0,"pair_risk":0,"ask_road_conflict":0,"road_chaos":0,"table_side_history":0,"remaining_rank_pressure":0,"remaining_rank_total":0.30}'::jsonb
        when 'bankerPair' then '{"tie_count":0,"banker_pair_count":0.20,"player_pair_count":0,"bead_road":0,"big_road":0,"big_eye_road":0,"small_road":0,"cockroach_road":0,"next_banker_road":0,"next_player_road":0,"shoe":0,"round":0,"shoe_stage":0.20,"player_point":0,"banker_point":0,"point_diff":0,"banker_natural":0,"player_natural":0,"banker_dragon":0,"player_dragon":0,"super_six":0,"tie_risk":0,"pair_risk":0.35,"ask_road_conflict":0,"road_chaos":0,"table_side_history":0.10,"remaining_rank_pressure":0.15,"remaining_rank_total":0}'::jsonb
        when 'playerPair' then '{"tie_count":0,"banker_pair_count":0,"player_pair_count":0.20,"bead_road":0,"big_road":0,"big_eye_road":0,"small_road":0,"cockroach_road":0,"next_banker_road":0,"next_player_road":0,"shoe":0,"round":0,"shoe_stage":0.15,"player_point":0,"banker_point":0,"point_diff":0,"banker_natural":0,"player_natural":0,"banker_dragon":0,"player_dragon":0,"super_six":0,"tie_risk":0,"pair_risk":0.20,"ask_road_conflict":0,"road_chaos":0,"table_side_history":0.25,"remaining_rank_pressure":0.20,"remaining_rank_total":0}'::jsonb
        else null end;
      if expected_current_weights is null or nullif(suggestion->>'id','') is null
         or suggestion->>'modelVersion' is distinct from 'v104-seven-head-shadow-v5-best-stage-side-reweight'
         or suggestion->>'searchMethod' is distinct from 'exhaustive_5_percent_grid'
         or suggestion->>'status' is distinct from 'pending'
         or suggestion->'autoApply' is distinct from 'false'::jsonb
         or jsonb_typeof(suggestion->'currentWeights') is distinct from 'object'
         or jsonb_typeof(suggestion->'suggestedWeights') is distinct from 'object'
         or jsonb_typeof(suggestion->'baselineMetrics') is distinct from 'object'
         or jsonb_typeof(suggestion->'candidateMetrics') is distinct from 'object'
         ...
         or coalesce(abs((select sum(value::numeric) from jsonb_each_text(suggestion->'suggestedWeights'))-1) > 0.000000001,true)
         or exists (
           select 1
      ...
      insert into public.v104_iteration_shadow_v5_weight_suggestions (
        ...
        suggestion->>'id',head_key_value,action_cycle_value,sample_start,sample_end,'v104-seven-head-shadow-v5-best-stage-side-reweight','exhaustive_5_percent_grid',
...
create or replace function public.review_v104_iteration_shadow_v5_suggestion(p_suggestion_id text, p_decision text, p_reviewer text)
returns jsonb language plpgsql security definer set search_path=pg_catalog,public,extensions as $$
declare reviewed public.v104_iteration_shadow_v5_weight_suggestions%rowtype;
begin
  perform 1 from public.v104_iteration_shadow_v5_runtime_settings
    where release_candidate='v104.5.0-seven-head-shadow.5' and enabled=true and status='shadow' and active_strategy_version='v104'
    for share;
  ...
  update public.v104_iteration_shadow_v5_weight_suggestions set status=p_decision,reviewed_by=p_reviewer,reviewed_at=now()
...
create or replace function public.begin_v104_iteration_shadow_v5_drain()
returns jsonb language plpgsql security definer set search_path=pg_catalog,public,extensions as $$
declare pending_count bigint; current_status text;
begin
  select status into current_status
  from public.v104_iteration_shadow_v5_runtime_settings
  where release_candidate='v104.5.0-seven-head-shadow.5' and enabled=true
  for update;
  if current_status not in ('shadow','draining') then
    raise exception 'v104 iteration shadow v5 is unavailable for drain';
  end if;
...
create or replace function public.finish_v104_iteration_shadow_v5_drain()
returns jsonb language plpgsql security definer set search_path=pg_catalog,public,extensions as $$
declare
...
begin
  ...
  if current_status is distinct from 'draining' then
    raise exception 'v104 iteration shadow v5 must be draining before disable';
  end if;
  select count(*) into pending_count
  from public.v104_iteration_shadow_v5_issuances i
  left join public.v104_iteration_shadow_v5_settlements s on s.prediction_id=i.id
  ...
  select count(*) into stored_reports from public.v104_iteration_shadow_v5_cycle_reports;
  select count(*) into stored_suggestions from public.v104_iteration_shadow_v5_weight_suggestions;
  if pending_count <> 0 or stored_reports < expected_reports or stored_suggestions < expected_suggestions then
    raise exception 'v104 iteration shadow v5 drain incomplete: pending %, reports %/%, suggestions %/%',
...
create or replace view public.v104_iteration_shadow_v5_history as
...
from public.v104_iteration_shadow_v5_issuances i left join public.v104_iteration_shadow_v5_settlements s on s.prediction_id=i.id
where i.strategy_version='v104-seven-head-shadow-v5-best-stage-side-reweight' and i.prediction_timing='pre_result_context' and i.prediction_issued_at is not null;

revoke all on table public.v104_iteration_shadow_v5_history from public,anon,authenticated,service_role;
grant select on table public.v104_iteration_shadow_v5_history to service_role;

revoke all on function public.issue_v104_iteration_shadow_v5_prediction(jsonb) from public,anon,authenticated;
revoke all on function public.settle_v104_iteration_shadow_v5_prediction(jsonb) from public,anon,authenticated;
revoke all on function public.persist_v104_iteration_shadow_v5_artifacts(jsonb,jsonb) from public,anon,authenticated;
revoke all on function public.review_v104_iteration_shadow_v5_suggestion(text,text,text) from public,anon,authenticated;
revoke all on function public.begin_v104_iteration_shadow_v5_drain() from public,anon,authenticated;
revoke all on function public.finish_v104_iteration_shadow_v5_drain() from public,anon,authenticated;
...
grant execute on function public.begin_v104_iteration_shadow_v5_drain() to service_role;
grant execute on function public.finish_v104_iteration_shadow_v5_drain() to service_role;
