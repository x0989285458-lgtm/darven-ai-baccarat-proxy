-- v101 latest-only additive RPC cutover. Apply before deploying the v101-only application.
-- Old entry points are intentionally retained until live cutover verification completes.

begin;

CREATE OR REPLACE FUNCTION public.apply_v101_rank_ledger_event(p_event jsonb, p_ledger jsonb DEFAULT NULL::jsonb) RETURNS jsonb
...
begin
  ...
  perform pg_catalog.pg_advisory_xact_lock(
    pg_catalog.hashtextextended('v101-rank|' || v_source || '|' || v_table || '|' || v_shoe, 0)
  );
...
revoke all on function public.apply_v101_rank_ledger_event(jsonb, jsonb) from public, anon, authenticated;
grant execute on function public.apply_v101_rank_ledger_event(jsonb, jsonb) to service_role;

create or replace function public.persist_v101_settled_round(
  p_roadmap jsonb,
  p_prediction jsonb
)
returns jsonb
...
begin
  if nullif(p_roadmap->>'source', '') is null
     ...
     or p_prediction->>'strategy_version' is distinct from 'v101'
...
create or replace function public.issue_v101_prediction(p_prediction jsonb)
returns jsonb
language plpgsql
security definer
set search_path = public
...
create or replace function public.settle_v101_prediction(p_roadmap jsonb, p_settlement jsonb)
returns jsonb
language plpgsql
security definer
set search_path = public
...
begin
  ...
  if existing.id is null
     or existing.strategy_version is distinct from 'v101'
...
create or replace function public.reconcile_v101_prediction_lifecycle(
  p_source text,
  p_table_id text,
  p_current_shoe text,
...
create or replace function public.get_v101_prediction_lifecycle_stats()
returns jsonb
language sql
stable
security definer
...
revoke all on function public.persist_v101_settled_round(jsonb, jsonb) from public, anon, authenticated;
grant execute on function public.persist_v101_settled_round(jsonb, jsonb) to service_role;
revoke all on function public.issue_v101_prediction(jsonb) from public, anon, authenticated;
grant execute on function public.issue_v101_prediction(jsonb) to service_role;
revoke all on function public.settle_v101_prediction(jsonb, jsonb) from public, anon, authenticated;
grant execute on function public.settle_v101_prediction(jsonb, jsonb) to service_role;
revoke all on function public.reconcile_v101_prediction_lifecycle(text, text, text, integer) from public, anon, authenticated;
grant execute on function public.reconcile_v101_prediction_lifecycle(text, text, text, integer) to service_role;
revoke all on function public.get_v101_prediction_lifecycle_stats() from public, anon, authenticated;
grant execute on function public.get_v101_prediction_lifecycle_stats() to service_role;

-- Cutover fence: keep v100 functions for rollback/history, but prevent predecessor writes.
...
create table if not exists public.v101_formal_release_previous_active (
  version text primary key,
  captured_at timestamptz not null default now()
);
alter table public.v101_formal_release_previous_active enable row level security;
...
do $$
begin
  if (select count(*) from public.v101_formal_release_previous_active) <> 1
     or not exists (select 1 from public.v101_formal_release_previous_active where version = 'v100') then
    raise exception 'v101 predecessor provenance must contain exactly v100';
  end if;
  if not exists (select 1 from public.ai_strategy_versions where version = 'v100') then
    raise exception 'v100 strategy source is required';
  end if;
end;
$$;

update public.ai_strategy_versions set status = 'archived' where status = 'active' and version <> 'v101';
...
select
  'v101', 'active', learned_from_date, 0, total_hit_rate,
  high_confidence_hit_rate, weights,
  jsonb_set(
    jsonb_set(
      jsonb_set(
        ...
        '{description}', to_jsonb('v101正式策略；主預測與權重沿用v100，副預測門檻依核准值調整。'::text), true
      ),
      '{main_strategy}', to_jsonb('v101_主預測沿用v100正式版'::text), true
    ),
    '{side_strategy}', to_jsonb('v101_副預測門檻調整版'::text), true
  ),
  'Only active runtime strategy and history source for formal release v101.',
  now(), now()
from public.ai_strategy_versions where version = 'v100'
...
begin
  if (select count(*) from public.ai_strategy_versions where status = 'active') <> 1
    ...
    raise exception 'v101 must be the only active strategy';
  end if;
end;
$$;
