...
describe('AI百家預測軟體', () => {
  ...
  it('renders the approved floating-portal member login without changing its two-field contract', async () => {
    ...
    expect(within(loginRegion).getByText('會員帳號', { selector: '.login-field-label' })).toBeVisible()
    expect(within(loginRegion).getByText('驗證密碼', { selector: '.login-field-label' })).toBeVisible()
