...
function proxyTablesFromMocks() {
  return mockTables.map((table, index) => ({
    ...
    prediction: {
      ...
      predictionId: `pid-${table.id}-${table.trend.current_round}`,
...
describe('AI百家預測軟體', () => {
  beforeEach(() => {
    vi.stubGlobal('fetch', vi.fn((url: string) => {
      ...
      if (url.includes('/api/status')) return Promise.resolve({ ok: true, status: 200, json: () => Promise.resolve({ connected: true, authenticated: true, tables: proxyTablesFromMocks(), statusText: '已抓到9桌', buildVersion: 'v104' }) })
  ...
  it('一般管理員不顯示也不請求影子預測資料', async () => {
    window.sessionStorage.setItem('darven-admin-account', 'manager001')
    window.sessionStorage.setItem('darven-admin-session-token', 'manager-session')
  ...
  it('shows stale data badge without treating old sourceUpdatedAt as realtime', async () => {
    vi.stubGlobal('fetch', vi.fn((url: string) => {
      ...
      if (url.includes('/api/status')) return Promise.resolve({ ok: true, status: 200, json: () => Promise.resolve({ connected: true, authenticated: true, tableCount: 9, statusText: '雲端資料過期，等待Worker重新抓牌', buildVersion: 'v104' }) })
      return Promise.resolve({ ok: true, status: 200, json: () => Promise.resolve({ items: [], reports: [], strategies: [] }) })
    }))
  ...
  it('clears the member session and returns to login on backend 401', async () => {
  ...
  it('keeps backend percentages without separate prediction or confidence copy', async () => {
    vi.stubGlobal('fetch', vi.fn((url: string) => {
      if (url.includes('/api/tables')) return Promise.resolve({ ok: true, status: 200, json: () => Promise.resolve(proxyTablesFromMocks().map((table, index) => ({
        ...table,
        prediction: { source: 'backend', predictedResult: index % 2 === 0 ? 'banker' : 'player', confidence: 34, strategyVersion: 'v104' },
      }))) })
      if (url.includes('/api/status')) return Promise.resolve({ ok: true, status: 200, json: () => Promise.resolve({ connected: true, authenticated: true, tables: [], statusText: '已抓到9桌', buildVersion: 'v104' }) })
      return Promise.resolve({ ok: true, status: 200, json: () => Promise.resolve({ items: [], reports: [], strategies: [] }) })
    }))
  ...
  it('shows settled predictions when real-card history is incomplete for the selected round', async () => {
    vi.stubGlobal('fetch', vi.fn((url: string) => {
      ...
      return Promise.resolve({ ok: true, status: 200, json: () => Promise.resolve({ connected: true, configured: true, buildVersion: 'v104' }) })
    }))
  ...
  it('clears old B-table rows immediately and ignores a late prior-table response', async () => {
  ...
  it('disables all actions and warns when the backend strategy is not current', async () => {
    vi.stubGlobal('fetch', vi.fn((url: string) => {
  ...
  it('waits and never takes side actions when backend side prediction data is missing', async () => {
    vi.stubGlobal('fetch', vi.fn((url: string) => {
      if (url.includes('/api/tables')) return Promise.resolve({ ok: true, status: 200, json: () => Promise.resolve(proxyTablesFromMocks().map((table) => ({ ...table, prediction: { source: 'backend', strategyVersion: 'v104', predictedResult: 'banker', confidence: 34 } }))) })
      if (url.includes('/api/status')) return Promise.resolve({ ok: true, status: 200, json: () => Promise.resolve({ connected: true, authenticated: true, tables: [], statusText: '已抓到9桌', buildVersion: 'v104' }) })
      return Promise.resolve({ ok: true, status: 200, json: () => Promise.resolve({ items: [], reports: [], strategies: [] }) })
    }))
    await renderApp()
  ...
  it('removes manual token connection controls and reads backend tables automatically', async () => {
    vi.stubGlobal('fetch', vi.fn((url: string) => {
      ...
      if (url.includes('/api/status')) return Promise.resolve({ ok: true, status: 200, json: () => Promise.resolve({ connected: true, authenticated: true, buildVersion: 'v104' }) })
      return Promise.resolve({ ok: true, status: 200, json: () => Promise.resolve({}) })
    }))
    await renderApp('/', false)
  ...
  it('shows only the approved ten member tables in exact identity order', async () => {
    ...
    vi.stubGlobal('fetch', vi.fn((url: string) => {
      ...
      if (url.includes('/api/online-license/status')) return Promise.resolve({ ok: true, status: 200, json: () => Promise.resolve({ configured: true }) })
      return Promise.resolve({ ok: true, status: 200, json: () => Promise.resolve({ connected: true, authenticated: true, buildVersion: 'v104' }) })
    }))
  ...
  it('shows a formal waiting state instead of mock tables when cloud data is empty', async () => {
    vi.stubGlobal('fetch', vi.fn((url: string) => {
      if (url.includes('/api/tables')) return Promise.resolve({ ok: true, status: 200, json: () => Promise.resolve([]) })
      if (url.includes('/api/status')) return Promise.resolve({ ok: true, status: 200, json: () => Promise.resolve({ connected: false, authenticated: false, statusText: 'MT自動登入未啟用，等待手動或Worker資料來源', buildVersion: 'v104' }) })
  ...
  it('admin create license and agent actions send real member/admin payloads to backend', async () => {
  ...
  it('overlays a green tie slash on banker/player big-road cells when a tie appears', async () => {
    vi.stubGlobal('fetch', vi.fn((url: string) => {
  ...
  it('keeps the selected table after proxy polling refreshes table data', async () => {
    vi.mocked(fetch).mockImplementation((input) => {
      if (String(input).includes('/api/status')) return Promise.resolve({
        ok: true, status: 200,
        json: () => Promise.resolve({ connected: true, authenticated: true, buildVersion: 'v104' }),
      } as Response)
  ...
  it('predicts only Banker or Player with confidence varied within 30-70 and no observe recommendation', () => {
  ...
  it('breaks exact main-score ties without defaulting to banker', () => {
    expect(calculatePrediction([
      { code: '02', outcome: 'Banker' },
      { code: '01', outcome: 'Player' },
  ...
  it('excludes ties from main prediction hit-rate scoring', () => {
  ...
  it('records every side prediction for learning but only counts action when each threshold is reached', () => {
    ...
    expect(isSidePredictionActionable('tie', 29)).toBe(false)
    expect(isSidePredictionActionable('tie', 30)).toBe(true)
    ...
    expect(isSidePredictionActionable('bankerPair', 50)).toBe(true)
    expect(isSidePredictionActionable('playerPair', 49)).toBe(false)
    expect(isSidePredictionActionable('playerPair', 50)).toBe(true)
    expect(isSidePredictionActionable('bankerDragon', 39)).toBe(false)
    expect(isSidePredictionActionable('bankerDragon', 40)).toBe(true)
    expect(isSidePredictionActionable('playerDragon', 39)).toBe(false)
    expect(isSidePredictionActionable('playerDragon', 40)).toBe(true)
  })


  it('dragon bonus follows main side and threshold', () => {
  ...
  it('gates super six and dragon bonus by main prediction', () => {
    const bankerBonus = {
      tie: 0,
      superSix: 95,
      bankerPair: 0,
  ...
  it('keeps side prediction weights independent per target with requested action rates', () => {
  ...
  it('detects road trends including single jump, double jump, long dragon, double dragon, and slopes', () => {
  ...
  it('uses high-hit weighted main and side prediction weights in the current frontend scorer', () => {
  ...
  it('main probability row uses five-road weighted score totals instead of mirroring confidence', () => {
    const prediction = evaluateFiveRoadPrediction({
      beadCells: [
        { code: '02', outcome: 'Banker' },
        { code: '01', outcome: 'Player' },
  ...
  it('report-facing prediction still hides internal source-weight hit rates from UI text', async () => {
    await renderApp()
    const prediction = screen.getByLabelText('AI預測結果')
    expect(within(prediction).queryByText(/AI預測:/)).not.toBeInTheDocument()
    expect(within(prediction).queryByText(/AI信心值/)).not.toBeInTheDocument()
  ...
  it('removes the free-version contact copy from member login', async () => {
  ...
  it('carries ask-road proxy payloads into frontend table trends', async () => {
    vi.mocked(fetch).mockImplementation((input) => {
      ...
      json: () => Promise.resolve(mockTables.map((table, index) => ({
        ...
        nextPlayerRaw: '222',
        prediction: {
          source: 'backend', strategyVersion: 'v104',
  ...
  it('member login calls online license API and enters frontend only after success', async () => {
  ...
  it('renders the approved floating-portal member login without changing its two-field contract', async () => {
    await renderApp('/login', false)

    const loginRegion = screen.getByLabelText('前台登入驗證')
    expect(loginRegion.closest('main')).toHaveClass('member-login-shell')
    expect(loginRegion.closest('main')).toHaveAttribute('data-ui-theme', 'navy-gold')
    ...
    expect(loginRegion.querySelectorAll('.login-field-icon')).toHaveLength(0)
    expect(loginRegion.querySelectorAll('.login-field-label')).toHaveLength(0)
    expect(screen.getByLabelText('會員帳號')).toHaveAttribute('placeholder', '請輸入會員帳號')
    expect(screen.getByLabelText('驗證密碼')).toHaveAttribute('placeholder', '請輸入驗證密碼')
    expect(loginRegion.querySelector('.login-message')).toHaveAttribute('aria-live', 'polite')
    expect(loginRegion.querySelector('.login-footer')).toBeInTheDocument()
    expect(loginRegion.querySelector('.system-status')).toHaveTextContent('系統狀態檢查中')
    expect(within(loginRegion).getByText('安全連線・資料加密')).toBeVisible()
  ...
  it('admin shows latest auto-synced 300-round test report metrics from memory center', async () => {
    vi.stubGlobal('fetch', vi.fn((url: string) => {
  ...
  it('admin shows strategy comparison, weak-table analysis, and next-version suggestions', async () => {
    vi.stubGlobal('fetch', vi.fn((url: string) => {
  ...
  it('applies requested admin layout, search, checkbox, fixed-agent, and 30-day limit behavior', async () => {
  ...
  it('keeps admin logout and changes member inactivity logout to 30 minutes', async () => {
    vi.useFakeTimers()
    window.sessionStorage.setItem('darven-admin-account', 'DVAI')
  ...
  it('clears a member session at the exact backend expiry even after recent activity', async () => {
    vi.useFakeTimers()
