...
describe('AI百家預測軟體', () => {
  beforeEach(() => {
    vi.stubGlobal('fetch', vi.fn((url: string) => {
      ...
      if (url.includes('/api/status')) return Promise.resolve({ ok: true, status: 200, json: () => Promise.resolve({ connected: true, authenticated: true, tables: proxyTablesFromMocks(), statusText: '已抓到9桌', buildVersion: 'v102' }) })
  ...
  it('shows stale data badge without treating old sourceUpdatedAt as realtime', async () => {
    vi.stubGlobal('fetch', vi.fn((url: string) => {
      ...
      if (url.includes('/api/status')) return Promise.resolve({ ok: true, status: 200, json: () => Promise.resolve({ connected: true, authenticated: true, tableCount: 9, statusText: '雲端資料過期，等待Worker重新抓牌', buildVersion: 'v102' }) })
      return Promise.resolve({ ok: true, status: 200, json: () => Promise.resolve({ items: [], reports: [], strategies: [] }) })
    }))
  ...
  it('keeps backend percentages without separate prediction or confidence copy', async () => {
    vi.stubGlobal('fetch', vi.fn((url: string) => {
      if (url.includes('/api/tables')) return Promise.resolve({ ok: true, status: 200, json: () => Promise.resolve(proxyTablesFromMocks().map((table, index) => ({
        ...table,
        prediction: { source: 'backend', predictedResult: index % 2 === 0 ? 'banker' : 'player', confidence: 34, strategyVersion: 'v102' },
      }))) })
      if (url.includes('/api/status')) return Promise.resolve({ ok: true, status: 200, json: () => Promise.resolve({ connected: true, authenticated: true, tables: [], statusText: '已抓到9桌', buildVersion: 'v102' }) })
      return Promise.resolve({ ok: true, status: 200, json: () => Promise.resolve({ items: [], reports: [], strategies: [] }) })
    }))
  ...
  it('shows settled predictions when real-card history is incomplete for the selected round', async () => {
    vi.stubGlobal('fetch', vi.fn((url: string) => {
      ...
      return Promise.resolve({ ok: true, status: 200, json: () => Promise.resolve({ connected: true, configured: true, buildVersion: 'v102' }) })
    }))
  ...
  it('waits and never takes side actions when backend side prediction data is missing', async () => {
    vi.stubGlobal('fetch', vi.fn((url: string) => {
      if (url.includes('/api/tables')) return Promise.resolve({ ok: true, status: 200, json: () => Promise.resolve(proxyTablesFromMocks().map((table) => ({ ...table, prediction: { source: 'backend', strategyVersion: 'v102', predictedResult: 'banker', confidence: 34 } }))) })
      if (url.includes('/api/status')) return Promise.resolve({ ok: true, status: 200, json: () => Promise.resolve({ connected: true, authenticated: true, tables: [], statusText: '已抓到9桌', buildVersion: 'v102' }) })
      return Promise.resolve({ ok: true, status: 200, json: () => Promise.resolve({ items: [], reports: [], strategies: [] }) })
    }))
    await renderApp()
  ...
  it('removes manual token connection controls and reads backend tables automatically', async () => {
    vi.stubGlobal('fetch', vi.fn((url: string) => {
      ...
      if (url.includes('/api/status')) return Promise.resolve({ ok: true, status: 200, json: () => Promise.resolve({ connected: true, authenticated: true, buildVersion: 'v102' }) })
      return Promise.resolve({ ok: true, status: 200, json: () => Promise.resolve({}) })
    }))
    await renderApp('/', false)
  ...
  it('shows only the approved ten member tables in exact identity order', async () => {
    ...
    vi.stubGlobal('fetch', vi.fn((url: string) => {
      ...
      if (url.includes('/api/online-license/status')) return Promise.resolve({ ok: true, status: 200, json: () => Promise.resolve({ configured: true }) })
      return Promise.resolve({ ok: true, status: 200, json: () => Promise.resolve({ connected: true, authenticated: true, buildVersion: 'v102' }) })
    }))
  ...
  it('shows a formal waiting state instead of mock tables when cloud data is empty', async () => {
    vi.stubGlobal('fetch', vi.fn((url: string) => {
      if (url.includes('/api/tables')) return Promise.resolve({ ok: true, status: 200, json: () => Promise.resolve([]) })
      if (url.includes('/api/status')) return Promise.resolve({ ok: true, status: 200, json: () => Promise.resolve({ connected: false, authenticated: false, statusText: 'MT自動登入未啟用，等待手動或Worker資料來源', buildVersion: 'v102' }) })
  ...
  it('keeps the selected table after proxy polling refreshes table data', async () => {
    vi.mocked(fetch).mockImplementation((input) => {
      if (String(input).includes('/api/status')) return Promise.resolve({
        ok: true, status: 200,
        json: () => Promise.resolve({ connected: true, authenticated: true, buildVersion: 'v102' }),
      } as Response)
  ...
  it('carries ask-road proxy payloads into frontend table trends', async () => {
    vi.mocked(fetch).mockImplementation((input) => {
      ...
      json: () => Promise.resolve(mockTables.map((table, index) => ({
        ...
        nextPlayerRaw: '222',
        prediction: {
          source: 'backend', strategyVersion: 'v102',
