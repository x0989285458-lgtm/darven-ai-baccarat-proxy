...
function liveTable(overrides: Record<string, unknown> = {}) {
  return {
    ...
    sourceUpdatedAt: new Date().toISOString(), buildVersion: 'v105',
    prediction: {
      source: 'backend', strategyVersion: 'v105', buildVersion: 'v105', targetTableId: 'BAG01', targetShoe: '88', targetRound: 13,
...
function history() {
  return {
    ok: true, buildVersion: 'v105', tableId: 'BAG01', shoe: 88,
    settledPredictions: [
...
async function renderMember(row = liveTable()) {
  vi.stubGlobal('fetch', vi.fn((url: string) => {
    ...
    if (url.includes('/api/status')) return Promise.resolve({ ok: true, status: 200, json: async () => ({ connected: true, authenticated: true, tableCount: 1, buildVersion: 'v105' }) })
...
describe('frozen frontend release', () => {
  ...
  it('uses exact v100 build and strategy contracts', () => {
    expect(frontendBuildMetadata).toEqual({ buildVersion: 'v105', strategyVersion: 'v105' })
  })
