...
describe('responsive history and road CSS', () => {
  ...
  it('uses the untouched pure background asset and approved command-deck geometry', () => {
    ...
    expect(css).toMatch(/\.dashboard-middle-grid\s*>\s*\.prediction-card,[\s\S]*?\.dashboard-history-panel\s*\{[^}]*height:\s*410px[^}]*min-height:\s*410px/s)
    expect(css).toMatch(/\.dashboard-middle-grid\s+\.prediction-card\s*\{[^}]*padding-bottom:\s*12px[^}]*overflow:\s*visible/s)
    expect(css).toMatch(/\.prediction-reason-card\s*\{[^}]*width:\s*min\(100%,\s*710px\)[^}]*margin:\s*4px\s+auto\s+0[^}]*justify-self:\s*center/s)
    ...
    expect(css).toMatch(/Approved A command-deck[\s\S]*?\.member-dashboard\s+\.member-dashboard-header\s+\.header-meta\s*\{[^}]*margin-left:\s*auto/s)
    expect(css).toMatch(/Approved A command-deck[\s\S]*?\.member-dashboard\s+\.member-dashboard-header\s+\.status\s*\{[^}]*width:\s*10px[^}]*height:\s*10px[^}]*padding:\s*0[^}]*font-size:\s*0/s)
    expect(css).not.toMatch(/\.status\[title\]::after/)
