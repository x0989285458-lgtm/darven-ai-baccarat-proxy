...
describe('LiveRoadClient status messages', () => {
  ...
  it('explains the formal prediction only from non-zero pre-result sources supporting the issued direction', () => {
    const table = {
      prediction: {
        source: 'backend', strategyVersion: 'v105', predictedResult: 'banker', confidence: 48,
  ...
  it('uses the exact formal v105 big-road cycle reason instead of the generic roadmap label', () => {
    const table = {
      prediction: {
        source: 'backend', strategyVersion: 'v105', predictedResult: 'banker', confidence: 48,
        featureWeights: { roadmap_trend_signals: 0.275 },
        scoreSources: { roadmap_trend_signals: { banker: 0.55, player: 0.45 } },
        diagnostics: { roadCycles: { main: { detected: true, direction: 'banker', reasonText: '大路週期1－3連續2次，2路輔助確認，下一位置支持莊' } } },
      },
    }
    expect(backendPredictionReasonsFromTable(table as any)).toEqual([
      { key: 'roadmap_trend_signals', text: '大路週期1－3連續2次，2路輔助確認，下一位置支持莊', weight: 0.275 },
    ])
  })
  ...
  it('reports proxy running but MT not connected when status endpoint has no tables', async () => {
    ...
    vi.stubGlobal('fetch', vi.fn((url: string) => {
      if (url.endsWith('/api/status')) return Promise.resolve({ ok: true, json: () => Promise.resolve({ connected: false, authenticated: null, tables: [], buildVersion: 'v105' }) })
  ...
  it('reports token authenticated but still waiting when MT is connected without table rows', async () => {
    ...
    const statuses: Array<{ state: string; message: string }> = []
    vi.stubGlobal('fetch', vi.fn((url: string) => {
      if (url.endsWith('/api/status')) return Promise.resolve({ ok: true, json: () => Promise.resolve({ connected: true, authenticated: true, tables: [], buildVersion: 'v105' }) })
      return Promise.resolve({ ok: true, json: () => Promise.resolve([]) })
    }))
  ...
  it('surfaces proxy capture source statusText when available', async () => {
    ...
    const statuses: Array<{ state: string; message: string }> = []
    vi.stubGlobal('fetch', vi.fn((url: string) => {
      if (url.endsWith('/api/status')) return Promise.resolve({ ok: true, json: () => Promise.resolve({ connected: true, statusText: 'Chrome已連接，等待MT登入驗證', buildVersion: 'v105' }) })
      return Promise.resolve({ ok: true, json: () => Promise.resolve([]) })
    }))
  ...
  it('preserves rich MT road, ask-road and table context fields for AI scoring', async () => {
    ...
    const received: any[] = []
    vi.stubGlobal('fetch', vi.fn((url: string) => {
      if (url.endsWith('/api/status')) return Promise.resolve({ ok: true, json: () => Promise.resolve({ connected: true, authenticated: true, tableCount: 1, buildVersion: 'v105' }) })
      return Promise.resolve({ ok: true, json: () => Promise.resolve([{
  ...
  it('fails closed when backend status reports stale despite fresh table timestamps', async () => {
    ...
    const statuses: Array<{ state: string; message: string }> = []
    vi.stubGlobal('fetch', vi.fn((url: string) => {
      if (url.endsWith('/api/status')) return Promise.resolve({ ok: true, json: () => Promise.resolve({ connected: true, authenticated: true, statusText: '雲端資料 stale，等待Worker更新', buildVersion: 'v105' }) })
      return Promise.resolve({ ok: true, json: () => Promise.resolve([{
        tableId: 'BAG01', displayName: 'MT百家樂第1桌', tableType: 'BAC', round: 1,
        beadPlateRaw: '0102', bigRoadRaw: '0102', sourceUpdatedAt: new Date().toISOString(),
      }]) })
  ...
  it('uses each SSE heartbeat as an independent status refresh signal', async () => {
    vi.useFakeTimers()
    let streamController: ReadableStreamDefaultController<Uint8Array>
    const fetchMock = vi.fn((url: string) => {
      if (url.endsWith('/api/status')) return Promise.resolve({ ok: true, status: 200, json: () => Promise.resolve({ connected: true, authenticated: true, tableCount: 10, buildVersion: 'v105' }) })
    ...
    const statusesBeforeSecondHeartbeat = fetchMock.mock.calls.filter(([url]) => String(url).endsWith('/api/status')).length
    streamController!.enqueue(new TextEncoder().encode('event: heartbeat\ndata: {}\n\n'))
    await vi.advanceTimersByTimeAsync(1)
    const statusesAfterSecondHeartbeat = fetchMock.mock.calls.filter(([url]) => String(url).endsWith('/api/status')).length
    await vi.advanceTimersByTimeAsync(2999)
    client.disconnect(false)
    streamController!.close()

    expect(statusesAfterSecondHeartbeat).toBe(statusesBeforeSecondHeartbeat + 1)
  })

  it('keeps refreshing status while heartbeats continue without a tables event', async () => {
    vi.useFakeTimers()
    let streamController!: ReadableStreamDefaultController<Uint8Array>
    const fetchMock = vi.fn((url: string) => {
    ...
    client.connect()
    let heartbeatStatusRefreshes = 0
    for (let elapsed = 0; elapsed < 16000; elapsed += 3000) {
      const before = fetchMock.mock.calls.filter(([url]) => String(url).endsWith('/api/status')).length
      streamController.enqueue(new TextEncoder().encode('event: heartbeat\ndata: {}\n\n'))
      await vi.advanceTimersByTimeAsync(1)
      const after = fetchMock.mock.calls.filter(([url]) => String(url).endsWith('/api/status')).length
      if (after > before) heartbeatStatusRefreshes += 1
      await vi.advanceTimersByTimeAsync(2999)
    }
    client.disconnect(false)
    streamController.close()

    expect(heartbeatStatusRefreshes).toBe(6)
  })
