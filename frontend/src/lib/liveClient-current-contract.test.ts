...
describe('live frontend contract', () => {
  ...
  it('rejects polling rollback but a reconstructed client accepts its own first payload', async () => {
    vi.useFakeTimers()
    vi.setSystemTime(new Date('2026-07-16T02:00:30.000Z'))
    const proxy = (stamp: string) => ({ tableId: 'BAG01', tableType: 'BAC', shoe: 123, round: 18, beadPlateRaw: '0102', bigRoadRaw: '0102', sourceUpdatedAt: stamp, buildVersion: 'v106', prediction: validTable().prediction })
    let streamCall = 0
